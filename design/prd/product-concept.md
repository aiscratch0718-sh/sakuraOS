...
| **Anxiety of the new solution** | 作業員(特にベテラン): スマホ入力に抵抗。"覚える操作" が増えると反発。 → ゲーミフィ + さくらししまるナビでオンボーディング負担を最小化。 |
...
4. 業務終了 → REPORT3 入力(3 タップ + スライダー数本)
   ...
   - 写真添付(任意 / 自動 GPS スタンプ)
5. 工具返却 → QR スキャン → 返却登録
6. さくらししまるナビが「+10XP / +5SC」を表示 → ランク確認
...
- 作業員(特に高齢層)が REPORT3 のスマホ入力を嫌がるリスク → さくらししまるナビ + ゲーミフィ + 入力時間 3分以内に収める設計で対応
