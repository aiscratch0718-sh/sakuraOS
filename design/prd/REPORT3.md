...
10. **保存** → 「+10XP / +5SC を獲得しました!」とさくらししまるナビが表示
...
- **TR-RPT-012**: SHALL 提出時に `idempotency_key`(クライアント生成 UUID v4)を
  受け取り、再送時に重複適用しない
- **TR-RPT-013**: SHALL ゲーミフィ通知(さくらししまるナビ表示)は **トランザクション
...
- **TR-RPT-018**: SHALL 提出後にさくらししまるナビが「+XP / +SC / ランクアップ(該当時)」を
  3秒間トースト表示する
...
| Edge Case | Behavior |
|-----------|----------|
| **Empty (新規作業員、初回)** | サンプル付きの空フォーム。さくらししまるナビが「最初の入力で +20XP ボーナス」案内 |
