...
export function QuestBadgeSummary({
  ...
  return (
    <div
      className="dashboard-quest-summary relative grid h-full grid-cols-3 gap-1.5 overflow-hidden rounded-md"
      style={{
        backgroundImage:
    ...
    >
      ...
      <div className="dashboard-quest-card flex flex-col rounded-md bg-white p-2 shadow-sm">
        ...
        <div className="mt-1 text-[11px] font-bold text-slate-800">Lv. {level}</div>
      ...
      <div className="dashboard-quest-card flex flex-col rounded-md bg-white p-2 shadow-sm">
        ...
        <div className="mt-1 truncate text-[11px] font-black text-slate-950">{questLabel}</div>
          ...
          {BADGES.map((b) => {
            ...
            return (
              <div key={b.id} className="flex items-start gap-1.5">
                <Icon
                  className={`mt-0.5 h-4 w-4 flex-shrink-0 ${b.color}`}
                  aria-hidden
                />
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-[8px] leading-tight text-slate-500">
                    {b.note}
                  </div>
                </div>
                <span className="flex-shrink-0 whitespace-nowrap text-[8px] leading-tight text-slate-400">
                  {b.date}
                </span>
              </div>
            );
