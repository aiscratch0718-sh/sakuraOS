...
PHASE 1: CONCEPT
 ...
 /brainstorm ──────────────────────────────────────────────────► design/prd/product-concept.md
 /setup-stack ────────────────────────────────────────────────► CLAUDE.md + technical-preferences.md
 /design-review [product-concept.md] ────────────────────────────► concept validated
 /gate-check ─────────────────────────────────────────────────► PASS → advance to systems-design
 │
 ▼
PHASE 2: SYSTEMS DESIGN
 /map-systems ────────────────────────────────────────────────► design/prd/systems-index.md
 │
 ▼ (for each system, in dependency order)
 /design-system [name] ──────────────────────────────────────► design/prd/[system].md
 /design-review [system].md ─────────────────────────────────► per-PRD review comments
 │
 ▼ (after all MVP PRDs done)
 /review-all-prds ────────────────────────────────────────────► design/prd/prd-cross-review-[date].md
...
PHASE 3: TECHNICAL SETUP
 ...
 /create-permissions-manifest ────────────────────────────────────► docs/architecture/control-manifest.md
 /gate-check ─────────────────────────────────────────────────► PASS → advance to pre-production
 │
 ▼
PHASE 4: PRE-PRODUCTION
 [UX — before epics, so specs exist when stories are written]
 /ux-design [screen/dashboard/patterns] ────────────────────────────► design/ux/*.md
 ...
 /test-helpers ───────────────────────────────────────────────► tests/helpers/[framework-specific].gd
 ...
 /usability-test-report ────────────────────────────────────────────► tests/usability test/vertical-slice.md
...
PHASE 5: PRODUCTION (repeating sprint loop)
 ...
 /content-audit ──────────────────────────────────────────────► PRD content gaps identified
 ...
 /team-core-workflows / /team-content / /team-ui / /team-information-architecture / /team-notifications
...
PHASE 6: POLISH
 ...
 /tradeoff-check ──────────────────────────────────────────────► feature tradeoff report + fixes
...
How a single PRD gets authored, reviewed, and handed to architecture:

```
systems-index.md (input)
product-concept.md (input)
upstream PRDs (input, if any)
 │
 ▼
/design-system [name]
 │
 ├── Pre-check: feasibility table + framework risk flags
 │
 ├── Section cycle × 8:
 │ question → options → decision → draft → approval → WRITE
 │ [each section written to file immediately after approval]
 │
 └── Output: design/prd/[system].md (complete, all 8 sections)
 │
 ▼
 /design-review design/prd/[system].md
 │
 ...
 │
 ▼ (after all MVP PRDs + cross-review)
 /review-all-prds
 │
 └── Output: prd-cross-review-[date].md
```
...
```
design/prd/*.md (UI/UX requirements extracted)
design/user-journey.md (emotional arc, if authored)
 │
 ▼
/ux-design dashboard → design/ux/dashboard.md
...
/ux-review design/ux/
 ...
 ├── Phase 2: visual design (design-director)
 ├── Phase 3: layout implementation (frontend-engineer)
...
/story-readiness [story]
 ...
 /dev-story [story]
 │
 ├── Reads: story file, linked PRD requirement, ADR decisions, control manifest
 ├── Routes to: feature-engineer / platform-engineer / frontend-engineer / etc.
 │
...
/test-helpers ──────────────────────────────────────────────────► tests/helpers/[framework].gd (GDUnit4, NUnit, etc.)

[Phase 5 — per-sprint QA cycle]
/qa-plan [sprint or feature]
 │
 ├── Reads: story files, PRDs, acceptance criteria
 ├── Classifies each story by test type:
 │ Logic → automated unit test (BLOCKING)
 │ Integration → integration test or documented usability test (BLOCKING)
...
```
design/prd/*.md (UX requirements extracted)
design/user-journey.md (emotional arc)
 │
 ▼
/ux-design dashboard → design/ux/dashboard.md
/ux-design screen [name] → design/ux/screens/[name].md
...
/adopt
 ...
 │
 ▼
 /design-system retrofit [path] → fills missing PRD sections
...
| Have a concept, no framework | `/setup-stack` |
| Have concept + framework | `/map-systems` |
| Mid-systems design | `/design-system [next system]` or `/map-systems next` |
| All PRDs done | `/review-all-prds` → `/gate-check` |
| In technical setup | `/create-architecture` → `/architecture-decision` |
| Starting UX design | `/ux-design screen [name]` or `/ux-design dashboard` |
