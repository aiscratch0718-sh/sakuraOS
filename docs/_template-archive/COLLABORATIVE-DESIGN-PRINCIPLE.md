...
User = Product Director (Final Decision Maker)
...
❌ User just says "make a product" and waits
```
 ...
 **Option A: Full Randomized Discovery**
 ...
 - Example products: Minecraft early, Doodle God
 ...
 **Option B: Ingredient Hint System**
 ...
 - Example products: Potion Craft, Opus Magnum
 ...
 **Option C: Hybrid (Discovery + Hints)**
 ...
 - ✅ Accessible to multiple user types
 - ✅ Rewards persistence
...
5. AGENT DRAFTS BASED ON DECISION
 ...
 ### Core Loop
 1. User selects 2-4 ingredients from inventory
...
6. USER REVIEWS AND REFINES
 ...
 so users aren't frustrated by complex recipes."
...
10. FINAL APPROVAL BEFORE WRITING
 ...
 ✓ Overview, ✓ User Fantasy, ✓ Detailed Rules, ✓ Formulas,
 ✓ Edge Cases, ✓ Dependencies, ✓ Tuning Knobs, ✓ Acceptance Criteria

 May I write this to design/prd/crafting-system.md?"

 User: "Yes, write it." ← ONLY NOW does file get created

11. AGENT WRITES FILE
 Agent: [Uses Write tool]
 "Created design/prd/crafting-system.md. Would you like me to run
...
**Example:** "Design the billing system"

```
❌ WRONG (Autonomous):
User: "Design the billing system"
Agent: [Writes design/prd/billing-system.md with full design]
Agent: "Done! Here's your billing system."

✅ RIGHT (Collaborative):
User: "Design the billing system"
Agent: "I'd love to help design your billing system. First, some questions:
 ...
 4. Any reference products that nail the feel you want?"

User: [Answers questions]
Agent: [Presents 3 billing approaches with pros/cons]
User: [Picks one, refines it]
Agent: [Drafts mechanics section]
User: [Reviews, requests changes]
Agent: [Iterates until user approves]
Agent: "Ready to write to design/prd/billing-system.md?"
User: "Yes"
Agent: [Writes file]
```
...
Agent: [Writes src/features/billing/discount-calculator.ts]
Agent: "Implemented!"

✅ RIGHT:
User: "Implement damage calculation per design/prd/billing-system.md"
Agent: [Reads design doc]
 "I see the design calls for:
 - Base damage * tool multiplier * crit multiplier * resistance
 ...
 2. Where should tool/resistance data live? (CharacterStats? Equipment class?)
 3. Do we need damage events for VFX/UI sound cues hookup?
...
Agent: [Writes code, runs through user flow-code rule checks, fixes issues]
 "Implemented src/features/billing/discount-calculator.ts.
 The user flow-code rule flagged one issue: multiplier was hardcoded.
 I moved it to src/config/billing/discount-config.ts.
...
Skill: "I'll help you brainstorm roguelike concepts using professional
 ...
 2. Any sub-product category? (action, deckbuilder, puzzle, tactics?)
 ...
 - Traditional (billing-focused)
 - Hybrid (billing + other verb)
 - Unconventional (non-billing primary loop)
...
```
"Should the system show users a warning before destructive actions?
...
```
"The design doc doesn't specify what happens when a user dies while crafting.
 Some options:
...
❌ **Too Open-Ended:**
```
"What should the billing system be like?"
← Too broad, user doesn't know where to start
```

❌ **Leading/Assuming:**
```
"I'll make billing real-time since that's standard for this product category."
...
← No pros/cons, no reference to product pillars
```
...
AskUserQuestion:
 ...
 description: "Users discover by trying combinations — high mystery"
 - label: "system actor/Book Learning"
...
AskUserQuestion:
 ...
 - label: "Cut Entirely"
 description: "Drop crafting, focus on billing — deadline met, pillar missing"
```
...
```
[product-manager returns 3 billing approaches with analysis]

Orchestrator uses AskUserQuestion:
 question: "Which billing approach should we develop?"
...
1. Agent creates file with skeleton (all section headers, empty bodies)
 Agent: "May I create design/prd/crafting-system.md with the section skeleton?"
 User: "Yes"
...
Agent: "This implementation requires changes to 3 files:
 1. src/user flow/inventory.ts (core logic)
 2. src/ui/inventory_ui.ts (display)
...
### ✅ Experts Who Explain
- "I recommend Option A because [reasoning with product design theory]"
...
- ❌ "I've designed your billing system [done]"
...
Team skills (like `/team-core-workflows`) orchestrate multiple agents, but still collaborative:

```
User: "/team-core-workflows 'bulk record export'"

Skill (Coordinator):
"I'll coordinate the core-workflows team to design and implement bulk export.
 Before we start, a few questions:

 1. Is this a mobility tool, a billing tool, or both?
 2. Should it support filtered exports, full-table exports, or both?
 3. Any reference products? (Just Cause, Bionic Commando, Attack on Titan?)

 [User answers]

 Based on your answers, I'll have the team propose options.

 **Phase 1: Design (product-manager)**
 Starting design phase...
 [product-manager asks questions, presents options]
 [User makes decisions]
 product-manager: 'Design complete. Proceeding to architecture phase.'

 **Phase 2: Architecture (feature-engineer)**
 [feature-engineer proposes code structure]
 [User approves or requests changes]

 **Phase 3: Parallel Implementation**
 I'll now coordinate 4 agents to implement in parallel:
 - feature-engineer: Core grapple physics
 - design-systems-engineer: Cable VFX
 - interaction-designer: Whoosh + impact UI sound cues
 - ml-engineer: Edge case reactions to being grappled
...
"All 4 subsystems implemented. Would you like me to:
 A) Have feature-engineer integrate them now
...
```
"Create a billing system" ← No guidance, agent forced to guess
...
3. Reference product design theory, user's pillars, or comparable products
