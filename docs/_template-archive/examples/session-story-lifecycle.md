...
**Context:** Solo indie dev, mid-production sprint. The movement system PRD and architecture are complete. A story for movement's core implementation has been generated by `/create-stories`.

**Starting State:**
- `design/prd/movement-system.md` — complete, all 8 sections
...
> - PRD referenced: `design/prd/movement-system.md` ✅
> - PRD section coverage: Detailed Rules ✅, Formulas ✅, Edge Cases ✅
...
> - Ambiguities found: **1**
> - ⚠️ Story says "roll direction follows last input direction" but PRD says "roll in movement direction" — these conflict when user stops and immediately rolls. Clarify before pickup.
>
...
**User:**
> Roll uses last movement direction. If user is standing still, roll uses facing direction instead.
...
> Reading `production/stories/STORY-MOV-001.md`, `design/prd/movement-system.md`, and `docs/architecture/movement-adr-001.md`...
>
> **Architecture (from ADR-001):**
> - `MovementComponent.ts` — attached to user scene, not a singleton
...
> ```
> src/user flow/movement/
> movement_component.ts # CharacterBody2D logic, state machine
> terrain_modifier_table.ts # Const table: terrain_type -> {speed_mod, drain_mod}
> tests/unit/
> test_movement_formulas.ts # Unit tests for speed/drain calculations
> ```
>
...
**Agent:**
> Implementing `movement_component.ts`. Key excerpt for your review before I write the file:
>
> ```typescript
...
**Agent writes:**
- `src/user flow/movement/movement_component.ts`
- `src/user flow/movement/terrain_modifier_table.ts`
- `tests/unit/test_movement_formulas.ts`
...
>
> Suggest running `/code-review src/user flow/movement/` before marking complete. Not blocking.
>
> ---
...
>
> **Deferred criteria logged** (will auto-surface in dashboard integration story):
> - Status banner reflects degraded state (requires status-feed integration)
> - Live notification on threshold breach (requires notifications dispatcher)
>
> **Next ready story from sprint:**
> - STORY-BILL-002: Discount calculator — logical next story (downstream dep now unblocked)
> - STORY-DASH-001: Dashboard threshold badges — also unblocked
>
...
- **TR-ID references**: story stores `TR-MOV-001` IDs, not quoted PRD text — avoids false deviation flags when PRD wording is cleaned up later
