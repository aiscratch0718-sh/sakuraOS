# Upgrading Claude Code Web Studio

This guide covers upgrading your existing web product repo from one version
...
changed. Resolve each one — your product content goes in, structural improvements
...
your framework and project settings. Keep your content; accept the structural changes.
...
| Category | Changes |
|----------|---------|
| **New skill** | `/design-system-bible` — guided section-by-section visual identity authoring (9 sections). Mandatory design-director Task spawn per section. AD-ART-BIBLE sign-off gate. Required at Technical Setup phase. |
| **New skill** | `/asset-spec` — per-asset visual spec and AI generation prompt generator. Reads design system bible + PRD/level/character docs. Writes `design/assets/specs/` files and `design/assets/asset-manifest.md`. Full/lean/solo modes. |
| **New director gates (3)** | `AD-CONCEPT-VISUAL` (brainstorm Phase 4), `AD-ART-BIBLE` (design system bible sign-off), `AD-PHASE-GATE` (gate-check panel) |
| **`/brainstorm` update** | Added `Task` to allowed-tools (was missing — blocked all director spawning). Art-director now spawns in parallel with product-director after pillars lock. Visual Identity Anchor written to product-concept.md. |
| **`/gate-check` update** | Art-director added as 4th parallel director (AD-PHASE-GATE). Visual artifact checks: Visual Identity Anchor (Concept gate), design system bible (Technical Setup gate), AD-ART-BIBLE sign-off + character visual profiles (Pre-Production gate). |
| **`/team-information-architecture` update** | Art-director added to Step 1 parallel spawn (visual direction before layout). Level-designer now receives design-director targets as explicit constraints. Step 4 design-director role corrected to production-concepts only. |
| **`/team-content` update** | Art-director added to Phase 2 parallel spawn (character visual design, environmental storytelling, cinematic tone). |
| **`/design-system` update** | Routing table expanded with design-director + design-systems-engineer for Core feature interaction, UI, Dialogue, Animation/VFX, Character categories. Visual/Audio section now mandatory (with design-director Task spawn) for 7 system categories. |
| **`workflow-catalog.yaml`** | `/design-system-bible` added to Technical Setup (required). `/asset-spec` added to Pre-Production (optional, repeatable). |

### Files: Safe to Overwrite

**New files to add:**
```
.claude/skills/design-system-bible/SKILL.md
...
.claude/skills/team-information-architecture/SKILL.md
.claude/skills/team-content/SKILL.md
...
**Key themes:** Director gates system, gate intensity modes, Next.js C# specialist
...
| **New agent** | `typescript-specialist` — C# code quality in Next.js projects |
| **Skill updates (13)** | All gate-using skills now parse `--review [full\|lean\|solo]` and include it in their argument-hint: `brainstorm`, `map-systems`, `design-system`, `architecture-decision`, `create-architecture`, `create-epics`, `create-stories`, `sprint-plan`, `milestone-review`, `usability-test-report`, `prototype`, `story-done`, `gate-check` |
| **`/start` update** | Added Phase 3b — sets review mode during onboarding, writes `production/review-mode.txt` |
| **`/setup-stack` update** | Language selection step for Next.js (TypeScript vs C#) |
...
.claude/skills/usability-test-report/SKILL.md
...
```
/design-system billing --review lean
/gate-check concept --review full
/brainstorm my-product-idea --review solo
```
...
**Key themes:** Product category-agnostic agents, new skills, skill fixes

### What Changed

| Category | Changes |
|----------|---------|
| **New skills (1)** | `/consistency-check` — cross-PRD entity consistency scanner |
| **Skill fixes (all team-*)** | Added no-argument guards, formal `Verdict: COMPLETE / BLOCKED` keywords, per-step AskUserQuestion gates, adjacent area dependency checks (team-information-architecture), ethics enforcement (team-growth), NO-GO path with Phase skip (team-release) |
| **Agent fixes (4)** | Product category-agnostic language in product-manager, systems-analyst, business-analyst, growth-engineer — removed RPG-specific terms |
...
**Existing files to overwrite (no user content):**
```
.claude/skills/team-core-workflows/SKILL.md ← no-arg guard, verdict keywords, gate improvements
.claude/skills/team-content/SKILL.md ← no-arg guard, verdict keywords, gate improvements
...
.claude/skills/team-notifications/SKILL.md ← no-arg guard, verdict keywords, gate improvements
.claude/skills/team-information-architecture/SKILL.md ← no-arg guard, verdict keywords, adjacent area checks
.claude/skills/team-growth/SKILL.md ← no-arg guard, verdict keywords, ethics enforcement
...
.claude/agents/product-manager.md ← product category-agnostic language
.claude/agents/systems-analyst.md ← product category-agnostic language
.claude/agents/business-analyst.md ← product category-agnostic language
.claude/agents/growth-engineer.md ← product category-agnostic language
```
...
| Category | Changes |
|----------|---------|
| **New skills (17)** | `/ux-design`, `/ux-review`, `/help`, `/quick-design`, `/review-all-prds`, `/story-readiness`, `/story-done`, `/sprint-status`, `/adopt`, `/create-architecture`, `/create-permissions-manifest`, `/create-epics`, `/create-stories`, `/dev-story`, `/propagate-design-change`, `/content-audit`, `/architecture-review` |
| **New skills QA (12)** | `/qa-plan`, `/smoke-check`, `/soak-test`, `/regression-suite`, `/test-setup`, `/test-helpers`, `/test-evidence-review`, `/test-flakiness`, `/skill-test`, `/bug-triage`, `/team-growth`, `/team-qa` |
...
| **New templates (8)** | `ux-spec.md`, `dashboard-design.md`, `accessibility-requirements.md`, `interaction-pattern-library.md`, `user-journey.md`, and adoption plan templates |
...
| **PRD template** | `## Product Feel` section added (input responsiveness, animation targets, impact moments) |
...
.claude/skills/team-growth/SKILL.md
...
.claude/skills/story-readiness/SKILL.md
.claude/skills/story-done/SKILL.md
.claude/docs/templates/product-design-document.md ← adds Product Feel section
README.md
docs/WORKFLOW-GUIDE.md
UPGRADING.md
```
...
.claude/agents/design-director.md ← adds memory: project
.claude/agents/brand-director.md ← adds memory: project
.claude/agents/business-analyst.md ← adds memory: project
.claude/agents/product-manager.md ← adds memory: project
.claude/agents/feature-engineer.md ← adds memory: project
.claude/agents/lead-engineer.md ← adds memory: project
.claude/agents/screen-designer.md ← adds memory: project
.claude/agents/content-director.md ← adds memory: project
.claude/agents/systems-analyst.md ← adds memory: project
.claude/agents/design-systems-engineer.md ← adds memory: project
.claude/agents/frontend-engineer.md ← adds memory: project
.claude/agents/ux-designer.md ← adds memory: project
.claude/agents/information-architect.md ← adds memory: project
```
...
- **`/story-readiness`** — validates a story is implementation-ready before a developer picks it up. Checks Design (PRD req linked), Architecture (ADR accepted), Scope (criteria testable), and DoD (manifest version current). Verdict: READY / NEEDS WORK / BLOCKED.
- **`/story-done`** — 8-phase completion review after implementation. Verifies each acceptance criterion, checks for PRD/ADR deviations, prompts code review, updates the story file to `Status: Complete`, and surfaces the next ready story.

Flow: `/story-readiness` → implement → `/story-done` → next story

#### Full UX/UI Pipeline

- **`/ux-design`** — guided section-by-section UX spec authoring. Three modes: screen / flow, dashboard, or interaction pattern library. Reads PRD UI requirements and user journey. Output to `design/ux/`.
- **`/ux-review`** — validates UX specs against PRD alignment, accessibility tier, and pattern library. Verdict: APPROVED / NEEDS REVISION / MAJOR REVISION.
- **`/team-ui`** updated: Phase 1 now runs `/ux-design` + `/ux-review` as a hard gate before visual design begins.

#### Brownfield Adoption

**`/adopt`** onboards existing projects to the template format. Audits internal structure of PRDs, ADRs, stories, systems-index, and infra. Classifies gaps (BLOCKING/HIGH/MEDIUM/LOW). Builds an ordered migration plan. Never regenerates existing artifacts — only fills gaps.
...
- **`/test-setup`** — scaffolds the test framework and CI/CD pipeline for your framework
- **`/test-helpers`** — generates framework-specific test helper libraries (GDUnit4, NUnit, etc.)
...
- **`/soak-test`** — generates a soak test protocol for extended use sessions (stability, memory leaks)
- **`/regression-suite`** — maps test coverage to PRD critical paths, identifies fixed bugs lacking regression tests
...
#### Team Growth-Ops and Team QA Orchestration

- **`/team-growth`** — coordinates growth-engineer + business-analyst + customer-success-manager + analytics-engineer for post-launch content planning (seasonal events, battle pass, retention)
- **`/team-qa`** — orchestrates qa-lead + qa-engineer + feature-engineer + engineering manager through a full QA cycle: strategy, execution, coverage, and sign-off
...
4. **Run `/adopt`** if you have existing PRDs or ADRs that predate this template version — it will identify which sections need to be added without overwriting your content.
...
**Key themes:** `/design-system` PRD authoring, `/map-systems` rename, custom status line
...
| Category | Changes |
|----------|---------|
| **New skills** | `/design-system` (guided PRD authoring, section-by-section) |
...
| **Skill updates** | `brainstorm`, `start`, `design-review`, `project-stage-detect`, `setup-stack` — cross-reference fixes |
...
```
ctx: 42% | claude-sonnet-4-6 | Production | Core feature interaction System > Melee Core feature interaction > Hitboxes
```
...
| Category | Changes |
|----------|---------|
| **New skills** | `/start` (onboarding), `/map-systems` (systems decomposition), `/design-system` (guided PRD authoring) |
...
.claude/skills/project-stage-detect/SKILL.md
.claude/skills/setup-stack/SKILL.md
.claude/skills/team-notifications/SKILL.md
.claude/skills/team-core-workflows/SKILL.md
.claude/skills/team-information-architecture/SKILL.md
.claude/skills/team-content/SKILL.md
...
- The `## Technology Stack` section (your framework/language choices)
...
If you ran `/setup-stack`, this file has your framework config, naming
