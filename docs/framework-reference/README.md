...
tech stack(s) used in this project. These files exist because **LLM knowledge
has a cutoff date** and tech stacks update frequently.

## Why This Exists

Claude's training data has a knowledge cutoff (currently May 2025). Product engines
like Next.js, React, and NestJS ship updates that introduce breaking API changes,
...
Framework-specialist agents are instructed to:

1. Read `VERSION.md` to confirm the current framework version
2. Check `deprecated-apis.md` before suggesting any framework API
...
- After upgrading the framework version
...
1. Update `VERSION.md` with the new framework version and date
