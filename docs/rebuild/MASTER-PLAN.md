...
> ⚠️ 各サービスの API キー/認証情報を **畠中様→クライアント(秋元様)** から
...
## P8-09: Claude API ハイブリッド統合(さくらししまる AI 化)

> **本タスクは ADR-0002 で詳細仕様を確定済**。
> `docs/architecture/adr-0002-shishimaru-ai-integration.md` を参照すること。
>
> ⚠️ **本タスクは全 110 タスクの最後の方に着手**(畠中様確定方針: 有料 API 系統合は最後)。
> 着手直前に畠中様 → クライアント秋元様 へのメリット / デメリット / リスク範囲の
> 説明資料を Claude が再提示する義務あり。

畠中様の確認事項(クライアント説明前提):
- AI による個人評価 → 秋元様 OK
- 横展開差別化 → 強く志向
- データ秘匿性 → 担保設計でクリア可能との見立て

採用方針: **ハイブリッドアーキテクチャ**
- ルールベース層 = 高頻度・定型・安全関連(常時稼働、フォールバック兼)
- Claude API 層 = 低頻度・分析・自由質問(コーチング / 経営層自由質問 / 月次レポート所感)

サブタスク:

### P8-09a: PII 匿名化レイヤー(`src/lib/ai/sanitize.ts`)
- 氏名 / メール / 電話 / 住所(精密)/ GPS(精密)/ マイナンバー / 給与額 → 送信禁止
- user_id / department_id を安定ハッシュ化
- 応答受信後に表示名へ rehydrate するヘルパー
- 単体テスト必須(送信禁止項目が混入しないことを assert)

### P8-09b: Claude API クライアント(`src/lib/anthropic/`)
- `@anthropic-ai/sdk` を導入
- API キーは Vercel 環境変数 `ANTHROPIC_API_KEY` で管理
- system prompt はバージョン管理(`prompts/shishimaru.v1.ts`)
- temperature 0.3 / top_p 0.9 / max_tokens 300 で固定
- timeout 5 秒
- モデル使い分け:Haiku(コーチング) / Sonnet(自由質問・月次)

### P8-09c: フォールバック構造 + 監査ログ(migration 0017)
- `shishimaru_log` テーブル新設(ADR-0002 項 6 参照)
- レート制限 / 月予算上限の実装(ADR-0002 項 7)
- API 失敗時に自動でルールベースへ落ちる(ユーザー側からは見えない劣化)
- キャッシュレイヤー(同 context 5 分 TTL)

### P8-09d: コーチング機能(ステータス画面)
- `/pc/profile/status` の獅子丸アドバイス枠を「ルール+API ハイブリッド」へ
- 「もっと詳しく」ボタンで API 呼出
- 「これは AI による分析です」表示必須

### P8-09e: 自由質問機能(管理職向け)
- `/pc/ai-assistant`(office+ のみ)
- 「今週の傾向は?」等の自由質問入力
- 過去質問履歴の表示 + 共有

### P8-09f: 月次レポート所感(月次バッチ)
- pg_cron で月初に各テナントの月次サマリーを生成
- 経営層向けレポートにコメント所感を AI で追加
...
### P8-09h: クライアント説明資料の再提示(着手直前マイルストーン)
**実装着手前に Claude が必ず以下を整理して畠中様に提示する**:
1. メリット(横展開差別化、コーチング、自由質問の具体価値)
2. デメリット(コスト、レイテンシ、ハルシネーション、リグレッションリスク)
3. リスク範囲(送信データ一覧、匿名化方式、Anthropic 規約抜粋、保持期間、障害時挙動)
4. オプトイン / オプトアウト設計
5. コスト・予算管理
6. 法的・倫理的論点(AI 評価の説明責任、労務上の注意点)
7. 段階導入計画とロールバック手順

→ 秋元様への打ち合わせ材料として畠中様が活用
...
> 畠中様の追加要件「Google Drive 風のフォルダ管理 + ロール別アクセス制御」を実装。
...
- 2026-05-10: **ADR-0002(さくらししまる AI 統合)を策定**。Claude API ハイブリッド
