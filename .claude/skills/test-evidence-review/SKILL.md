...
**Integration stories**: Glob `tests/integration/[system]/[story-slug]_test.*`
 - Also check `production/session-logs/` for usability test records mentioning the story
...
check, verify, or framework-specific assertion patterns). Low assertion count is
...
For Logic stories where the PRD has a Formulas section: check that the test
