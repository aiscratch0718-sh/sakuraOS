...
- **frontend-engineer** — UI framework, screens, widgets, data binding, implementation
- **design-director** — Visual style, layout polish, consistency with design system bible
- **framework UI specialist** — Validates UI implementation patterns against framework-specific best practices (read from `.claude/docs/technical-preferences.md` Framework Specialists → UI Specialist)
...
- `dashboard-design.md` — Dashboard-specific UX specification (multi-widget summary surfaces)
...
- `subagent_type: frontend-engineer` — UI framework, screens, widgets, data binding
- `subagent_type: design-director` — Visual style, layout polish, design system bible consistency
- `subagent_type: [UI framework specialist]` — Framework-specific UI pattern validation (e.g., component-library-specialist, admin-ui-specialist, nextjs-specialist)
...
- `design/prd/product-concept.md` — platform targets and intended audience
- `design/user-journey.md` — user's state and context when they reach this screen
- All PRD UI Requirements sections relevant to this feature
...
- (b) Proceed without the pattern library — frontend-engineer will treat all patterns created as new and add each to a new `design/ux/interaction-patterns.md` at completion

Do NOT invent or assume patterns from the feature name or PRD alone. If the user chooses (b), explicitly instruct frontend-engineer in Phase 3 to treat all patterns as new and document them in `design/ux/interaction-patterns.md` when implementation is complete. Note the pattern library status (created / absent / updated) in the final summary report.

Summarize the context in a brief for the ux-designer: what the user is doing, what they need, what constraints apply, and which existing patterns are relevant.
...
If designing a dashboard, use the `dashboard-design.md` template instead of `ux-spec.md`.

> **Notes on special cases:**
> - For dashboard design specifically, invoke `/ux-design` with `argument: dashboard` (e.g., `/ux-design dashboard`).
...
Delegate to **design-director**:
- Review the full UX spec (flows, wireframes, interaction patterns, accessibility notes) — not just the wireframe images
- Apply visual treatment from the design system bible: colors, typography, spacing, animation style
...
- Specify all asset requirements: icons at specified sizes, illustrations, fonts, decorative elements — with precise dimensions and format requirements
- Ensure consistency with existing implemented UI screens
- Output: visual design spec with style notes and asset manifest

### Phase 3: Implementation

Before implementation begins, spawn the **framework UI specialist** (from `.claude/docs/technical-preferences.md` Framework Specialists → UI Specialist) to review the UX spec and visual design spec for framework-specific implementation guidance:
- Which UI approach should be used for this screen? (e.g., Server Component vs Client Component split in Next.js; component library choice in React + Node; module + admin-UI choice in NestJS)
- Any framework-specific gotchas for the proposed layout or interaction patterns?
- Recommended component / hook / module structure for the framework?
- Output: framework UI implementation notes to hand off to frontend-engineer before they begin
...
- **UI NEVER owns or directly mutates server state** — render from server data; trigger mutations through Server Actions / API calls
- All user-facing text through the localization system — no hardcoded user-facing strings
- Full keyboard support (Tab, Enter, Esc, arrow keys where applicable); mouse and touch supported equally
- Implement accessibility features per the committed tier in `design/accessibility-requirements.md`
- Wire up data binding to server state via the project's chosen data-fetching layer
...
- **ux-designer**: Verify implementation matches wireframes and interaction spec. Test keyboard-only navigation and pointer paths. Check accessibility features function correctly.
- **design-director**: Verify visual consistency with design system bible. Check at minimum and maximum supported resolutions.
...
- Verify animations are skippable and respect the user's motion reduction preferences
...
- **Confirm all dashboard widgets respect the visual budget** defined in `design/ux/dashboard.md` (widget count, layout zones, density target)

## Quick Reference — When to Use Which Skill

- `/ux-design` — Author a new UX spec for a screen, flow, or dashboard from scratch
...
delegated to sub-agents and sub-skills (`/ux-design`, `frontend-engineer`). Each enforces the
