...
layer, extracted from all Accepted ADRs, technical preferences, and framework
...
### Framework Reference
- Read `docs/framework-reference/[framework]/VERSION.md` for framework + version
- Read `docs/framework-reference/[framework]/deprecated-apis.md` — these become
 forbidden API entries
- Read `docs/framework-reference/[framework]/current-best-practices.md` if it exists

Report: "Loaded [N] Accepted ADRs, framework: [name + version]."
...
### Framework API Constraints (from "Framework Compatibility" section)
- Post-cutoff APIs that require verification
- Verified behaviours that differ from default LLM assumptions
- API fields or methods that behave differently in the pinned framework version

### Layer Classification
Classify each rule by the architectural layer of the system it governs:
- **Foundation**: Scene management, event architecture, save/load, framework init
- **Core**: Core core user flows, main user systems, physics/collision
- **Feature**: Secondary systems, secondary mechanics, AI
- **Presentation**: Rendering, audio, UI, VFX, stylesheets
...
- Framework-recommended patterns → Required entries
...
Pass: the Control Manifest Preview from Phase 4 (rule counts per layer, full extracted rule list), the list of ADRs covered, framework version, and any rules sourced from technical-preferences.md or framework reference docs.
...
> **Status**: [Active — regenerate with `/create-permissions-manifest update` when ADRs change]
...
technical preferences, and framework reference docs. For the reasoning behind each
...
*Applies to: scene management, event architecture, save/load, framework initialisation*
...
*Applies to: core core user flow, main user systems, physics, collision*
...
*Applies to: rendering, audio, UI, VFX, stylesheets, animations*
...
### Forbidden APIs ([framework version])
These APIs are deprecated or unverified for [framework + version]:
- `[api name]` — deprecated since [version] / unverified post-cutoff
- Source: `docs/framework-reference/[framework]/deprecated-apis.md`
...
4. **Source every rule** — never add a rule that doesn't trace to an ADR, a
 technical preference, or a framework reference doc
