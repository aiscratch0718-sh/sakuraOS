---
name: dev-story
description: "Read a story file and implement it. Loads the full context (story, PRD requirement, ADR guidelines, control manifest), routes to the right programmer agent for the system and framework, implements the code and test, and confirms each acceptance criterion. The core implementation skill — run after /story-readiness, before /code-review and /story-done."
...
| Control manifest | `docs/architecture/control-manifest.md` | **WARN and continue** — "Control manifest not found — layer rules cannot be checked. Run `/create-permissions-manifest`." |
...
- **TR-ID** — the PRD requirement identifier
...
PRD requires now. Do not rely on any inline text in the story file (may be stale).
...
- The Framework Compatibility section (post-cutoff APIs, known risks)
...
- `Framework:` value — determines which programmer agents to use
...
If the story's Type is `Config/Data`, no programmer agent or framework specialist is needed. Jump directly to Phase 4 (Config/Data note). The implementation is a data file edit — no routing table evaluation, no framework specialist.

### Primary agent routing table

| Story context | Primary agent |
|---|---|
| Foundation layer — any type | `platform-engineer` |
| Any layer — Type: UI | `frontend-engineer` |
| Any layer — Type: Visual/Feel | `feature-engineer` (implements) |
| Core or Feature — user flow mechanics | `feature-engineer` |
| Core or Feature — AI behaviour, pathfinding | `ml-engineer` |
| Core or Feature — networking, replication | `api-engineer` |
| Config/Data — no code | No agent needed (see Phase 4 Config note) |

### Framework specialist — always spawn as secondary for code stories

Read the `Framework Specialists` section of `.claude/docs/technical-preferences.md`
to get the configured primary specialist. Spawn them alongside the primary agent
when the story involves framework-specific APIs, patterns, or the ADR has HIGH
framework risk.

| Framework | Specialist agents available |
|--------|----------------------------|
| Next.js | `nextjs-specialist`, `app-router-specialist`, `tailwind-specialist` |
| React | `react-specialist`, `component-library-specialist`, `css-animation-specialist` |
| NestJS | `nestjs-specialist`, `api-gateway-specialist`, `orm-specialist`, `admin-ui-specialist`, `websocket-specialist` |

**When framework risk is HIGH** (from the ADR or VERSION.md): always spawn the framework
specialist, even for non-framework-facing stories. High risk means the ADR records
assumptions about post-cutoff framework APIs that need expert verification.
...
2. The current PRD requirement text (from TR registry)
3. The ADR Decision + Implementation Guidelines (verbatim — do not summarise)
4. The control manifest rules for this layer
5. The framework naming conventions and performance budgets
6. Any framework-specific notes from the ADR Framework Compatibility section
...
Spawn `feature-engineer` to implement the code/animation calls. Note that
...
- Test the formula bounds from the PRD Formulas section
...
- Any framework-specific risks the specialist flagged
...
- [ ] [criterion] — DEFERRED: requires usability test (Visual/Feel)

**Deviations from scope**: [None] or [list files touched outside story boundary]
**Framework risks flagged**: [None] or [specialist finding]
...
- **Load before implementing** — do not start coding until all context is loaded
 (story, TR-ID, ADR, manifest, framework prefs). Incomplete context produces code
