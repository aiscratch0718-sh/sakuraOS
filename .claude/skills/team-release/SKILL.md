---
name: team-release
description: "Orchestrate the release team: coordinates release-manager, qa-lead, devops-engineer, and engineering manager to execute a release from candidate to deployment."
...
- **security-engineer** — Pre-release security audit (invoke if product has online/multiplayer features or user data)
- **analytics-engineer** — Verify telemetry events fire correctly and dashboards are live
- **customer-success-manager** — Patch notes, launch announcement, user-facing messaging
- **engineering manager** — Go/no-go decision, stakeholder communication, scheduling
...
- `subagent_type: customer-success-manager` — Patch notes and launch communication
- `subagent_type: engineering manager` — Go/no-go decision, stakeholder communication
- `subagent_type: api-engineer` — Netcode stability sign-off (invoke if product has multiplayer)
...
Delegate to **engineering manager**:
...
- **security-engineer**: Conduct pre-release security audit. Review authentication, multi-tenant isolation, dependency CVEs, secrets handling, and data privacy compliance. Sign off on security posture.
- **api-engineer** *(if a public API or partner integration is part of this release)*: Sign off on API contract stability. Verify versioning, idempotency, rate limits, and webhook signature handling under load.
...
- Run performance benchmarks against targets (delegate to **performance-engineer** if available)
- **analytics-engineer**: Verify all telemetry events fire correctly on release build. Confirm dashboards are receiving data. Check that critical funnels (onboarding, progression, engagement if applicable) are instrumented.
- Output: localization, performance, and analytics sign-off

### Phase 5: Go/No-Go
Delegate to **engineering manager**:
- Collect sign-off from: qa-lead, release-manager, devops-engineer, security-engineer (if spawned in Phase 3), api-engineer (if spawned in Phase 3), and technical-director
- Evaluate any open issues — are they blocking or can they ship?
- Make the go/no-go call
- Output: release decision with rationale

**If engineering manager declares NO-GO:**
...
- **Skip Phase 6 entirely** — do not tag, deploy to staging, deploy to production, or spawn customer-success-manager.
...
Delegate to **customer-success-manager** (in parallel with deployment):
- Finalize release notes using `/release-notes [version]`
- Prepare launch announcement (in-app banner, customer email, status page note, blog post, social media)
- Draft known issues post if any S3+ issues shipped
- Output: all user-facing release communication, ready to publish on deploy confirmation

### Phase 7: Post-Release
- **release-manager**: Generate release report (what shipped, what was deferred, metrics)
- **engineering manager**: Update milestone tracking, communicate to stakeholders
- **qa-lead**: Monitor incoming bug reports for regressions
- **customer-success-manager**: Publish all customer-facing communication, monitor support inbox and customer sentiment
...
All file writes (release checklists, changelogs, release notes, deployment scripts) are
