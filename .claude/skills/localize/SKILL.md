...
confuses users, and blocks platform certification. This skill covers the
...
- Suggest key names following the convention: `[category].[subcategory].[description]`
 - Example: `ui.dashboard.kpi_label`, `email.invoice.subject`, `nav.settings.title`
- Each new entry must include a `context` field — a translator comment explaining:
 - Where it appears (which screen, which scene)
 - Maximum character length
 - Any placeholder meaning (`{playerName}` = the user's chosen display name)
...
Read:
- `design/prd/` — extract product category, tone, setting, character names
...
## Product Overview
[2-3 paragraph summary of the product, product category, tone, and audience]

## Tone and Voice
- **Overall tone**: [e.g., "Darkly comic, not slapstick — think Terry Pratchett, not Looney Tunes"]
- **User address**: [e.g., "Second person, informal. Never formal 'vous' — always 'tu' for French"]
...
- [Product name]
- [UI terms that match in-framework labels]
- [Brand or trademark names]

## Placeholder Reference
| Placeholder | What it represents | Example |
|-------------|-------------------|---------|
| `{playerName}` | User's chosen display name | "Shadowblade" |
...
- Any lines that are responses in a conversation (provide context: "User just said X")
...
Read `.claude/docs/technical-preferences.md` to determine the framework. Then check:

**Layout mirroring**
- Is RTL layout enabled in the framework? (Next.js: `Control.layout_direction`, React: `RTL Support` package, NestJS: text direction flags)
...
- Framework-specific RTL flags in scene/prefab files
...
- The list of all screens/flows in the product (from `design/prd/` or `/content-audit` output)
...
1. **Functional string check** — every string displays in-app without truncation, placeholder errors, or encoding corruption
2. **UI overflow check** — translated strings that exceed UI bounds (even if within character limits, some languages expand)
3. **Contextual accuracy** — a sample of 10% of strings reviewed in-app for translation accuracy and natural phrasing
...
[ ] Engineering Manager approves shipping [Locale]
```
...
- Cultural review is required for any locale where the product will be sold commercially
