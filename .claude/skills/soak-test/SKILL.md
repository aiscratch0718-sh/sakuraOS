---
name: soak-test
description: "Generate a soak test protocol for extended use sessions. Defines what to observe, measure, and log during long use sessions to surface slow leaks, fatigue effects, and edge cases that only appear after sustained use. Primarily used in Polish and Release phases."
argument-hint: "[duration: 30m | 1h | 2h | 4h] [focus: memory | stability | feature tradeoff | all]"
user-invocable: true
...
A soak test (also called an endurance test) is an extended use session run
with specific observation goals. Unlike a smoke check (broad critical path,
~10 min) or a single-feature usability test (~30 min), a soak test runs for **30
...
- **Fun fatigue** — mechanics that feel good in a first session but grow
 repetitive over extended use
- **Content exhaustion** — the point where users run out of novel content
...
- When extended use has not been formally tracked
...
- `4h` — deep soak; required for products with long session design (RPGs, sims)

**Focus** (default: `all`):
- `memory` — focus on heap size, object count, leak patterns
- `stability` — focus on crash/freeze/hang detection
- `feature tradeoff` — focus on fun fatigue, content exhaustion, difficulty perception
...
Read:
- `.claude/docs/technical-preferences.md` — framework (for framework-specific memory
 monitoring guidance), performance budgets (memory ceiling, target FPS)
- `design/prd/product-concept.md` — intended session length (for comparison against
 soak duration), core user flow description
- Most recent file in `production/usability tests/` — prior usability test findings
...
Framework-specific monitoring guidance:
...
- [ ] All Dashboard widgets still rendering correctly
- [ ] Input responding as expected (no input loss or lag spike)

### Feature Tradeoff / fatigue observation items (if focus = feature tradeoff or all)
...
> **Date**: [date]
> **Duration**: [duration]
> **Focus**: [memory | stability | feature tradeoff | all]
> **Framework**: [framework]
...
- [ ] Product is running from a **fresh launch** (not resumed from a prior session)
- [ ] All background applications closed (minimise OS memory interference)
- [ ] Performance monitoring tool open and recording:
 - **Next.js**: Debugger → Monitors tab → Memory section visible
 - **React**: Memory Profiler window open
 - **NestJS**: `stat memory` ready in console
- [ ] Soak target confirmed: [session design intent from product concept]
- [ ] Prior known issues to watch for: [from most recent usability test / qa-plan]
...
|--------|---------------|
| Memory / Heap | [record before first frame of user flow] |
| Object Count | [record] |
| FPS (first 30 seconds) | [record] |
| [Framework-specific metric] | [record] |
...
**Feature Tradeoff / Fatigue** *(if applicable)*:
...
### Feature Tradeoff / Fatigue Summary
...
2. Record each checkpoint as you use
...
- **Duration should match the product's session design** — a 5-minute product
