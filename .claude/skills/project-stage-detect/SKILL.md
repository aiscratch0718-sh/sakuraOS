...
- Count PRD files in `design/prd/*.md`
...
- Count screen designs in `design/levels/`

**Source Code** (`src/`):
- Count source files (language-agnostic)
- Identify major systems (directories with 5+ files)
- Check for core/, user flow/, ai/, networking/, ui/ directories
...
| **Technical Setup** | Systems index exists, framework not configured |
| **Pre-Production** | Framework configured, `src/` has <10 source files |
...
- "I see billing code (`src/features/billing/`) but no `design/prd/billing.md`. Was this prototyped first, or should we reverse-document?"
