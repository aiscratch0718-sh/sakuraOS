---
name: create-epics
description: "Translate approved PRDs + architecture into epics — one epic per architectural module. Defines scope, governing ADRs, framework risk, and untraced requirements. Does NOT break into stories — run /create-stories [epic-slug] after each epic is created."
...
**When to run:** After `/create-permissions-manifest` and `/architecture-review` pass.
...
Grep all PRDs for their `## Summary` sections before reading anything fully:

```
Grep pattern="## Summary" glob="design/prd/*.md" output_mode="content" -A 5
```

For `layer:` or `[system-name]` modes: filter to only in-scope PRDs based on
the Summary quick-reference. Skip full-reading anything out of scope.

### Step 2b — Full document load (in-scope systems only)

Using the Step 2a grep results, identify which systems are in scope. Read full documents **only for in-scope systems** — do not read PRDs or ADRs for out-of-scope systems or layers.

Read for in-scope systems:

- `design/prd/systems-index.md` — authoritative system list, layers, priority
- In-scope PRDs only (Approved or Designed status, filtered by Step 2a results)
- `docs/architecture/architecture.md` — module ownership and API boundaries
- Accepted ADRs **whose domains cover in-scope systems only** — read the "PRD Requirements Addressed", "Decision", and "Framework Compatibility" sections; skip ADRs for unrelated domains
...
- `docs/framework-reference/[framework]/VERSION.md` — framework name, version, risk levels

Report: "Loaded [N] PRDs, [M] ADRs, framework: [name + version]."
...
**Framework Risk**: [LOW / MEDIUM / HIGH — highest risk among governing ADRs]
...
## 4b. Engineering Manager Epic Structure Gate
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `engineering manager` via Task using gate **PR-EPIC** (`.claude/docs/director-gates.md`).
...
Present the engineering manager's assessment. If UNREALISTIC, offer to revise epic boundaries (split overscoped or merge underscoped epics) before writing. If CONCERNS, surface them and let the user decide. Do not write epic files until the engineering manager gate resolves.
...
| ADR | Decision Summary | Framework Risk |
|-----|-----------------|-------------|
...
- All acceptance criteria from `design/prd/[filename].md` are verified
...
| Epic | Layer | System | PRD | Stories | Status |
|------|-------|--------|-----|---------|--------|
...
4. **No invention** — all content comes from PRDs, ADRs, and architecture docs
