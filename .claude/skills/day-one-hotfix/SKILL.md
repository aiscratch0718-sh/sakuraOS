---
name: day-one-hotfix
description: "Prepare a day-one hotfix for a product launch. Scopes, prioritises, implements, and QA-gates a focused patch addressing known issues discovered after gold master but before or immediately after public launch. Treats the patch as a mini-sprint with its own QA gate and rollback plan."
...
Every shipped product has a day-one hotfix. Planning it before launch day prevents
...
- When a pre-launch usability test surfaces must-fix issues after the release gate passed
...
**Output:** `production/releases/day-one-hotfix-[version].md`
...
Use `AskUserQuestion`:
- Prompt: "Based on open bugs and cert feedback, here is the proposed day-one hotfix scope. Does this look right?"
...
- Options: `[A] Approve this scope` / `[B] Adjust — I want to add or remove items` / `[C] No day-one hotfix needed`

If [C]: output "No day-one hotfix required. Proceed to `/launch-checklist`." Stop.

### Step 2c — Check total scope

Sum estimated effort. If total exceeds 1 day of work:
> "⚠️ Patch scope is [N hours] — this exceeds a safe day-one window. Consider deferring lower-priority items to patch 1.1. A bloated day-one hotfix introduces more risk than it removes."
...
- What user communication is required if a rollback occurs
...
1. Spawn `lead-engineer` via Task with:
...
2. The lead-engineer implements and runs targeted tests.
...
QA verdict must be PASS or PASS WITH WARNINGS before proceeding. If FAIL: scope the failing fix out of the day-one hotfix and defer to 1.1.
...
```markdown
# Day-One Hotfix: [Product Name] v[version]
...
**Rollback owner**: [user / engineering manager]

---

## Approvals Required Before Deploy

- [ ] lead-engineer: all fixes reviewed
- [ ] qa-lead: QA gate PASS confirmed
- [ ] engineering manager: deployment timing approved
...
[Draft for customer-success-manager to review before publishing]

[list user-facing changes in plain language]
```

Ask: "May I write this patch record to `production/releases/day-one-hotfix-[version].md`?"
...
1. Run `/release-notes` to generate the user-facing version of the release notes
...
- **User communication is part of the patch** — `/release-notes` is a required output, not optional
