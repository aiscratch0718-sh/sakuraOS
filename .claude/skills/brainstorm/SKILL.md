---
name: brainstorm
description: "Guided product concept ideation — from zero idea to a structured product concept document. Uses professional studio ideation techniques, user psychology frameworks, and structured creative exploration."
argument-hint: "[product category or theme hint, or 'open'] [--review full|lean|solo]"
user-invocable: true
...
1. **Parse the argument** for an optional product category/theme hint (e.g., `roguelike`,
...
2. **Check for existing concept work**:
 - Read `design/prd/product-concept.md` if it exists (resume, don't restart)
 - Read `design/prd/product-pillars.md` if it exists (build on established pillars)
 ...
 **Use `AskUserQuestion`** at key decision points throughout brainstorming:
 - Constrained taste questions (product category preferences, scope, team size)
...
Start by understanding the person, not the product. Ask these questions
conversationally (not as a checklist):

**Emotional anchors**:
- What's a moment in a product that genuinely moved you, thrilled you, or made
 you lose track of time? What specifically created that feeling?
- Is there a fantasy or power trip you've always wanted in a product but never
 quite found?

**Taste profile**:
- What 3 products have you spent the most time with? What kept you coming back?
...
- Do you prefer products that challenge you, relax you, tell you stories,
...
- Tab "Experience" — "What kind of experience do you most want users to have?" (Challenge & Mastery / Story & Discovery / Expression & Creativity / Relaxation & Flow)
- Tab "Timeline" — "What's your realistic development timeline?" (Weeks / Months / 1-2 years / Multi-year)
- Tab "Dev level" — "Where are you in your dev journey?" (First product / Shipped before / Professional background)
...
Start with the core user verb (build, fight, explore, solve, survive,
create, manage, discover) and build outward from there. The verb IS the product.

**Technique 2: Mashup Method**
Combine two unexpected elements: [Product category A] + [Theme B]. The tension between
the two creates the unique hook. (e.g., "farming sim + cosmic horror",
"CRM + spreadsheet ergonomics", "knowledge base + real-time collaboration")

**Technique 3: Experience-First Design (MDA Backward)**
Start from the desired user emotion (aesthetic goal from MDA framework:
...
- **Core Verb** (the single most common user action)
...
The core user flow is the beating heart of the product — if it isn't fun in
isolation, no amount of content or polish will save the product.
...
1. **Core action feel** — prompt: "What's the primary feel of the core action?" Generate 3-4 options that fit the concept's product category and tone, plus a free-text escape (`I'll describe it`).

2. **Key design dimension** — identify the most important design variable for this specific concept (e.g., world reactivity, pacing, user agency) and ask about it. Generate options that match the concept. Always include a free-text escape.
...
- What structures the moment-to-moment use into cycles?
- Where does "one more turn" / "one more run" psychology kick in?
- What choices does the user make at this level?

**Session Loop** (30-120 minutes):
- What does a complete session look like?
- Where are the natural stopping points?
- What's the "hook" that makes them think about the product when not playing?

**Progression Loop** (days/weeks):
- How does the user grow? (Power? Knowledge? Options? Story?)
- What's the long-term goal? When is the product "done"?
...
- **Relatedness**: How does the user feel connected (to characters,
 other users, or the world)?

---

### Phase 4: Pillars and Boundaries

Product pillars are used by real AAA studios (God of War, Hades, The Last of
...
- Prompt: "Do these pillars feel right for your product?"
...
**After pillars and anti-pillars are agreed, spawn BOTH `product-director` AND `design-director` via Task in parallel before moving to Phase 5. Issue both Task calls simultaneously — do not wait for one before starting the other.**

- **`product-director`** — gate **CD-PILLARS** (`.claude/docs/director-gates.md`)
 Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`design-director`** — gate **AD-CONCEPT-VISUAL** (`.claude/docs/director-gates.md`)
 Pass: product concept elevator pitch, full pillar set with design tests, target platform (if known), any reference products or visual touchstones the user mentioned.

Collect both verdicts, then present them together using a two-tab `AskUserQuestion`:
- Tab **"Pillars"**: present product-director feedback. Options mirror the standard CD-PILLARS handling — `Lock in as-is` / `Revise [specific pillar]` / `Discuss further`.
- Tab **"Visual anchor"**: present the design-director's 2-3 named visual direction options. Options: each named direction (one per option) + `Combine elements across directions` + `Describe my own direction`.

The user's selected visual anchor (the named direction or their custom description) is stored as the **Visual Identity Anchor** — it will be written into the product-concept document and becomes the foundation of the design system bible.

If the product-director returns CONCERNS or REJECT on pillars, resolve pillar issues before asking for the visual anchor selection — visual direction should flow from confirmed pillars.

---

### Phase 5: User Type Validation

Using the Bartle taxonomy and Quantic Foundry motivation model, validate
who this product is actually for:

- **Primary user type**: Who will LOVE this product? (Achievers, Explorers,
 Socializers, Competitors, Creators, Storytellers)
- **Secondary appeal**: Who else might enjoy it?
- **Who is this NOT for**: Being clear about who won't like this product is as
 important as knowing who will
- **Market validation**: Are there successful products that serve a similar
 user type? What can we learn from their audience size?
...
- **Target platform**: Use `AskUserQuestion` — "What platforms are you targeting for this product?"
 ...
 Record the answer — it directly shapes the framework recommendation and will be passed to `/setup-stack`.
 Note platform implications if relevant (e.g., mobile means React is strongly preferred; console means Next.js has limitations; web means Next.js exports cleanly).

- **Framework experience**: Use `AskUserQuestion` — "Do you already have a framework you work in?"
 Options: `Next.js` / `React` / `NestJS` / `No preference — help me decide`
 - If they pick a framework → record it as their preference and move on. Do NOT second-guess it.
 - If "No preference" → tell them: "Run `/setup-stack` after this session — it will walk you through the full decision based on your concept and platform target." Do not make a recommendation here.
- **Art pipeline**: What's the art style and how labor-intensive is it?
- **Content scope**: Estimate level/area count, item count, user flow hours
...
Pass: core user flow description, platform target, framework choice (or "undecided"), list of identified technical risks.
...
**After scope tiers are defined, spawn `engineering manager` via Task using gate PR-SCOPE (`.claude/docs/director-gates.md`).**
...
4. **Generate the product concept document** using the template at
 ...
 brainstorm conversation, including the MDA analysis, user motivation
 profile, and flow state design sections.

 **Include a Visual Identity Anchor section** in the product concept document with:
 ...
 This section is the seed of the design system bible — it captures the "everything must
 move" decision before it can be forgotten between sessions.

5. Use `AskUserQuestion` for write approval:
- Prompt: "Product concept is ready. May I write it to `design/prd/product-concept.md`?"
...
If yes, generate the document using the template at `.claude/docs/templates/product-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
...
6. **Suggest next steps** (in this order — this is the professional studio
 ...
 1. "Run `/setup-stack` to configure the framework and populate version-aware reference docs"
 2. "Run `/design-system-bible` to create the visual identity specification — do this BEFORE writing PRDs. The design system bible gates asset production and shapes technical architecture decisions (rendering, VFX, UI systems)."
 3. "Use `/design-review design/prd/product-concept.md` to validate concept completeness before going downstream"
 4. "Discuss vision with the `product-director` agent for pillar refinement"
 ...
 5. "Author per-system PRDs with `/design-system` — guided, section-by-section PRD writing for each system identified in step 4"
 6. "Plan the technical architecture with `/create-architecture` — produces the master architecture server-actions and Required ADR list"
 ...
 9. "Prototype the riskiest system with `/prototype [core-mechanic]` — validate the core user flow before full implementation"
 10. "Run `/usability-test-report` after the prototype to validate the core hypothesis"
...
7. **Output a summary** with the chosen concept's elevator pitch, pillars,
 primary user type, framework recommendation, biggest risk, and file path.

Verdict: **COMPLETE** — product concept created and handed off for next steps.
...
> **Context is approaching the limit (≥70%).** The product concept document is saved
> to `design/prd/product-concept.md`. Open a fresh Claude Code session to continue
...
After the product concept is written, follow the pre-production pipeline in order:
1. `/setup-stack` — configure the framework and populate version-aware reference docs
2. `/design-system-bible` — establish visual identity before writing any PRDs
3. `/map-systems` — decompose the concept into individual systems with dependencies
4. `/design-system [first-system]` — author per-system PRDs in dependency order
5. `/create-architecture` — produce the master architecture server-actions
