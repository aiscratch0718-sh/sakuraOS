...
A project can have PRDs, ADRs, and stories — and every format-sensitive skill
...
- **`gdds`**: PRD format compliance only
...
- `design/prd/product-concept.md` — concept exists?
- `design/prd/systems-index.md` — systems index exists?
- Count PRD files: `design/prd/*.md` (excluding product-concept.md and systems-index.md)
- Count ADR files: `docs/architecture/adr-*.md`
- Count story files: `production/epics/**/*.md` (excluding EPIC.md)
- `.claude/docs/technical-preferences.md` — framework configured?
- `docs/framework-reference/` — framework reference docs present?
...
- product-concept.md exists → Concept
...
Report: "Detected phase: [phase]. Found: [N] PRDs, [M] ADRs, [P] stories."
...
For each PRD file found, check for the 8 required sections by scanning headings:

| Required Section | Heading pattern to look for |
|---|---|
| Overview | `## Overview` |
| User Fantasy | `## User Fantasy` |
...
Also check: does each PRD have a `**Status**:` field in its header block?
...
| `## Framework Compatibility` | HIGH — post-cutoff API risk is unknown |
...
If `design/prd/systems-index.md` exists:
...
| Framework reference | `docs/framework-reference/[framework]/VERSION.md` | HIGH — ADR framework checks blind |
...
- Framework, Language, Rendering, Physics → HIGH if unconfigured (ADR skills fail)
...
framework not configured when ADRs exist.

**HIGH** — Will cause stories to be generated with missing safety checks, or
infrastructure bootstrapping will fail.
Examples: ADRs missing Framework Compatibility, PRDs missing Acceptance Criteria
...
Examples: PRDs missing Tuning Knobs or Formulas sections, stories missing TR-IDs,
sprint-status.yaml missing.

**LOW** — Retroactive improvements that are nice-to-have but not urgent.
Examples: Stories missing Manifest Version stamps, PRDs missing Open Questions section.
...
2. HIGH gaps next, infrastructure before PRD/ADR content (bootstrapping needs correct formats)
3. MEDIUM gaps ordered: PRD gaps before ADR gaps before story gaps (stories depend on PRDs and ADRs)
4. LOW gaps last
...
`/design-system retrofit design/prd/[filename].md`
...
3. Run `/create-permissions-manifest` → creates manifest with version stamp
...
Framework: [configured / NOT CONFIGURED]
PRDs audited: [N] ([X] fully compliant, [Y] with gaps)
...
> **Generated**: [date]
> **Project phase**: [phase]
> **Framework**: [name + version, or "Not configured"]
...
Run `/create-permissions-manifest`
**Time**: 30 min
...
- **Options**:
 ...
 - `Solo` — No director reviews at all. Maximum speed. Best for product jams, prototypes, or if reviews feel like overhead.
...
- "The most urgent gap is missing Acceptance Criteria in [N] PRD(s):
 ...
 - "Yes — add Acceptance Criteria to [PRD filename] now"
...
6. **Never regenerate existing artifacts** — only fill gaps in what exists;
 do not rewrite PRDs, ADRs, or stories that already have content
