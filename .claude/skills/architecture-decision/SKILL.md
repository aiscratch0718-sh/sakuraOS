...
2. Identify which template sections are present by scanning headings:
 ...
 - `## Framework Compatibility` — HIGH if missing: post-cutoff risk unknown
...
5. If yes:
 ...
 - For **Framework Compatibility**: read the framework reference docs (same as Step 0 below)
 and ask the user to confirm the domain. Then generate the table with verified data.
 - For **PRD Requirements Addressed**: ask — "Which PRD systems motivated this decision?
 What specific requirement in each PRD does this ADR address?"
...
> (e.g., `event-system-architecture`, `physics-framework-choice`)."

Use the user's response as the title, then proceed to Step 0.

---

## 0. Load Framework Context (ALWAYS FIRST)

Before doing anything else, establish the framework environment:

1. Read `docs/framework-reference/[framework]/VERSION.md` to get:
...
3. Read the corresponding module reference if it exists:
 `docs/framework-reference/[framework]/modules/[domain].md`

4. Read `docs/framework-reference/[framework]/breaking-changes.md` — flag any
 changes in the relevant domain that post-date the LLM's training cutoff.

5. Read `docs/framework-reference/[framework]/deprecated-apis.md` — flag any APIs
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
 ...
 ```
 ⚠️ FRAMEWORK KNOWLEDGE GAP WARNING
 Framework: [name + version]
 Domain: [domain]
 Risk Level: HIGH — This version is post-LLM-cutoff.

 Key changes verified from framework-reference docs:
 - [Change 1 relevant to this domain]
 - [Change 2]

 This ADR will be cross-referenced against the framework reference library.
 Proceed with verified information only — do NOT rely solely on training data.
 ```

 If no framework has been configured yet, prompt: "No framework is configured.
 Run `/setup-stack` first, or tell me which framework you are using."
...
Read related code, existing ADRs, and relevant PRDs from `design/prd/`.
...
State Ownership:
 ...
 → If this ADR reads or writes user health, it must use this interface.
...
gathered (PRDs read, framework reference loaded, existing ADRs scanned). Then present
...
Alternatives I'll consider:
 A) [option derived from framework reference]
 B) [option derived from PRD requirements]
 C) [option from common patterns]
PRD systems driving this: [list derived from context]
...
**After framework specialist and TD reviews return** (Step 4.5/4.6), if unresolved
...
| **References Consulted** | [List framework-reference docs read, e.g. `docs/framework-reference/nextjs/modules/physics.md`] |
...
| PRD System | Requirement | How This ADR Addresses It |
|------------|-------------|--------------------------|
| [system-name].md | [specific rule, formula, or performance constraint from that PRD] | [how this decision satisfies it] |
...
4.5. **Framework Specialist Validation** — Before saving, spawn the **primary framework specialist** via Task to validate the drafted ADR:
 - Read `.claude/docs/technical-preferences.md` `Framework Specialists` section to get the primary specialist
 - If no framework is configured (`[TO BE CONFIGURED]`), skip this step
 - Spawn `subagent_type: [primary specialist]` with: the ADR's Framework Compatibility section, Decision section, Key Interfaces, and the framework reference docs path. Ask them to:
 ...
 3. Identify framework-specific risks or gotchas not captured in the current ADR draft
 - If the specialist identifies a **blocking issue** (wrong API, deprecated approach, framework version incompatibility): revise the Decision and Framework Compatibility sections accordingly, then confirm the changes with the user before proceeding
...
- `solo` → skip. Note: "TD-ADR skipped — Solo mode." Proceed to Step 4.7 (PRD sync check).
- `lean` → skip (not a PHASE-GATE). Note: "TD-ADR skipped — Lean mode." Proceed to Step 4.7 (PRD sync check).
- `full` → spawn as normal.

4.6. **Technical Director Strategic Review** — After the framework specialist validation, spawn `technical-director` via Task using gate **TD-ADR** (`.claude/docs/director-gates.md`):
 - Pass: the ADR file path (or draft content), framework version, domain, any existing ADRs in the same domain
 - The TD validates architectural coherence (is this decision consistent with the whole system?) — distinct from the framework specialist's API-level check
 - If CONCERNS or REJECT: revise the Decision or Alternatives sections accordingly before proceeding

4.7. **PRD Sync Check** — Before presenting the write approval, scan all PRDs
referenced in the "PRD Requirements Addressed" section for naming inconsistencies
...
developers reading the PRD from implementing the wrong interface.
```
...
2. Check if all prerequisite ADRs are now written. If yes, include a "Start writing PRDs" option.
...
[N] Start writing PRDs — run `/design-system [first-undesigned-system]` (only show if all prerequisite ADRs are written)
[N+1] Stop here for this session
```

If there are no remaining priority ADRs and no undesigned PRD systems, offer only "Stop here" and suggest running `/architecture-review` in a fresh session.
