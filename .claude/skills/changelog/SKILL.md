...
Read completed design documents from `design/prd/` for any new features implemented during this period.
...
- **New Features**: Entirely new user flow systems, modes, or content
- **Improvements**: Enhancements to existing features, UX improvements, performance gains
- **Bug Fixes**: Corrections to broken behavior
- **Feature Tradeoff Changes**: Tuning of user flow values, difficulty, economy
...
## Phase 5: Generate User-Facing Changelog
...
Output both changelogs to the user. The internal changelog is the primary working document. The user-facing changelog is ready for customer-facing publication (in-app, blog, status page) after review.
...
- Use `/release-notes [version]` to generate a styled, saved version for public release.
...
- Feature Tradeoff changes should always include the design reasoning, not just the numbers
