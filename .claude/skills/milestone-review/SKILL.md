...
## Phase 3b: Engineering Manager Risk Assessment

**Review mode check** — apply before spawning PR-MILESTONE:
- `solo` → skip. Note: "PR-MILESTONE skipped — Solo mode." Present the Go/No-Go section without a engineering manager verdict.
- `lean` → skip (not a PHASE-GATE). Note: "PR-MILESTONE skipped — Lean mode." Present the Go/No-Go section without a engineering manager verdict.
- `full` → spawn as normal.

Before generating the Go/No-Go recommendation, spawn `engineering manager` via Task using gate **PR-MILESTONE** (`.claude/docs/director-gates.md`).
...
Present the engineering manager's assessment inline within the Go/No-Go section. The engineering manager's verdict (ON TRACK / AT RISK / OFF TRACK) informs the overall recommendation — do not issue a GO against an OFF TRACK engineering manager verdict without explicit user acknowledgement.
