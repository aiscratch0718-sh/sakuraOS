---
name: story-done
description: "End-of-story completion review. Reads the story file, verifies each acceptance criterion against the implementation, checks for PRD/ADR deviations, prompts code review, updates story status to Complete, and surfaces the next ready story from the sprint."
...
before the story is marked done, PRD and ADR deviations are explicitly
...
- **Story name and ID**
- **PRD Requirement TR-ID(s)** referenced (e.g., `TR-BILL-001`)
...
- **Framework notes** — any framework-specific constraints noted
...
- `docs/architecture/tr-registry.yaml` — look up each TR-ID in the story.
 ...
 source of truth for what the PRD required — do not use any requirement text
 that may be quoted inline in the story (it may be stale).
- The referenced PRD section — just the acceptance criteria and key rules, not
...
- **No hardcoded values check**: `Grep` for numeric literals in user flow code
...
- Criteria about subjective qualities ("feels responsive", "animations use correctly")
- Criteria about user flow behaviour ("user takes damage when...", "edge case responds to...")
...
- Criteria that require a full product build to test (end-to-end user flow scenarios)
- Mark as: `DEFERRED — requires usability test session`
...
1. Ask: is there a test — unit, integration, or confirmed manual usability test — that
...
|-----------|------|--------|
| AC-1: [criterion text] | tests/unit/test_foo.ts::test_bar | COVERED |
| AC-2: [criterion text] | Manual usability test confirmation | COVERED |
...
| **Integration** | Integration test in `tests/integration/[system]/` OR usability test doc | BLOCKING |
...
usability test record referencing this story.
...
1. **PRD rules check**: Using the current requirement text from `tr-registry.yaml`
 ...
 the PRD actually requires now — not what it required when the story was written.
 `Grep` the implemented files for key function names, data structures, or class
 names mentioned in the current PRD section.
...
4. **Hardcoded values check**: `Grep` the implemented files for numeric literals
 in user flow logic that should be in data files.
...
- **BLOCKING** — implementation contradicts the PRD or ADR (must fix before
 marking complete)
...
## Phase 5: Lead Engineer Code Review Gate
...
Spawn `lead-engineer` via Task using gate **LP-CODE-REVIEW** (`.claude/docs/director-gates.md`).

Pass: implementation file paths, story file path, relevant PRD section, governing ADR.
...
- [?] [Criterion 4] — DEFERRED: requires usability test
...
**Required evidence**: [unit test file | integration test or usability test | screenshot + sign-off | walkthrough doc | smoke check pass]
**Evidence found**: [YES — `[path]` | NO — BLOCKING | NO — ADVISORY]

### Deviations
[NONE] OR:
- BLOCKING: [description] — [PRD/ADR reference]
