...
Read `CLAUDE.md` for project context and the current tech stack. Understand what framework, language, and frameworks are in use so the prototype is built with compatible tooling.
...
## Phase 6: Product Director Review

**Review mode check** — apply before spawning CD-USABILITY-TEST:
- `solo` → skip. Note: "CD-USABILITY-TEST skipped — Solo mode." Proceed to Phase 7 summary with the prototyper's recommendation as the final verdict.
- `lean` → skip (not a PHASE-GATE). Note: "CD-USABILITY-TEST skipped — Lean mode." Proceed to Phase 7 summary with the prototyper's recommendation as the final verdict.
- `full` → spawn as normal.

Spawn `product-director` via Task using gate **CD-USABILITY-TEST** (`.claude/docs/director-gates.md`).

Pass: the full REPORT.md content, the original design question, product pillars and core fantasy from `design/prd/product-concept.md` (if it exists).

The product director evaluates the prototype result against the product's creative vision and pillars, then confirms, modifies, or overrides the prototyper's PROCEED / PIVOT / KILL recommendation. Their verdict is final. Update the REPORT.md `Recommendation` section if the product director's verdict differs from the prototyper's.

---

## Phase 7: Summary and Next Steps

Output a summary to the user: the core question, the result, the prototyper's initial recommendation, and the product-director's final decision. Link to the full report at `prototypes/[concept-name]/REPORT.md`.

If **PROCEED**: run `/design-system` to begin the production PRD for this mechanic, or `/architecture-decision` to record key technical decisions before implementation.
...
- **If PROCEED**: Run `/design-system [mechanic]` to author the production PRD, or `/architecture-decision` to record key technical decisions before implementation
...
- Run `/usability-test-report` to formally document any usability test sessions conducted during prototyping
