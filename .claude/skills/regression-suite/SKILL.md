...
description: "Map test coverage to PRD critical paths, identify fixed bugs without regression tests, flag coverage drift from new features, and maintain tests/regression-suite.md. Run after implementing a bug fix or before a release gate."
...
tests already in `tests/`** that collectively cover the product's critical paths
...
- `/regression-suite audit` — full audit of all PRD critical paths vs.
...
### Step 2c — Load PRD critical paths

For `audit` mode: read `design/prd/systems-index.md` to get all systems.
For each MVP-tier system, read its PRD and extract:
...
For `update` mode: skip full PRD scan. Instead read the current sprint plan
...
For each PRD acceptance criterion, determine whether a test exists:
...
- PRD sections added or revised since the regression suite was last updated
 (use Grep on PRD file modification hints if available, or ask the user)
...
> Coverage: [N]% of PRD critical paths

## How to run

[Framework-specific command to run all regression tests]
