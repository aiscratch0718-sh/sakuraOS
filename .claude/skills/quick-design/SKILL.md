---
name: quick-design
description: "Lightweight design spec for small changes — tuning adjustments, minor mechanics, feature tradeoff tweaks. Skips full PRD authoring when a system PRD already exists or the change is too small to warrant one. Produces a Quick Design Spec that embeds directly into story files."
...
This is the **lightweight design path** for changes that don't need a full PRD.
Full PRD authoring via `/design-system` is the heavyweight path. Use this skill
...
- **Tuning** — changing numbers or feature tradeoff values in an existing system with no
 behavioral change (most minimal path). Example: "increase jump height from 5
 to 6 units", "reduce edge case patrol speed by 10%".
...
- **New Small System** — a standalone feature small enough that it has no
 existing PRD and is under approximately one week of implementation work.
...
- Search `design/prd/` for the PRD most relevant to this change. Read the
 sections that this change would affect.
- Check whether `design/prd/systems-index.md` exists. If it does, read it to
...
Report what was found: "Found PRD at [path]. Relevant section: [section name].
...
**Type**: Tuning
**System**: [System name]
**PRD Reference**: `design/prd/[filename].md` — Tuning Knobs section
**Date**: [today]
...
Maps to PRD Tuning Knob: [knob name and its documented range].
...
**Type**: [Tweak / Addition]
**System**: [System name]
**PRD Reference**: `design/prd/[filename].md`
**Date**: [today]
...
[Why is this change needed? What user experience problem does it solve?
Reference the relevant MDA aesthetic or user feedback if applicable.]

## Design Delta

Current PRD says (quoting `design/prd/[filename].md`, [section]):

> [exact quote of the relevant rule or description]

This spec changes that to:

[New rule or description, written with the same precision as a PRD Detailed
...
|--------|--------|-----------------|
| [system] | [how it is affected] | [update PRD / update data file / no action] |
...
Use a trimmed PRD structure. Include only the sections that are directly
necessary — skip User Fantasy, full Formulas, and Edge Cases unless the
...
- [ ] [Experiential criterion: feels right — what a usability test validates]
- [ ] [Regression criterion: does not break adjacent system]

## Systems Index

This system is not currently in `design/prd/systems-index.md`.
...
If a PRD update is required (flagged in the spec), ask separately after
writing the quick spec:

"This spec modifies rules in [System Name]. May I update
`design/prd/[filename].md` — specifically the [section name] section?"

Show the exact text that would be changed (old vs. new) before asking. Do not
make PRD edits without explicit approval.
...
implementation. Reference this spec in the story's PRD Reference field.
```
...
Quick Design Specs **bypass** `/design-review` and `/review-all-prds` by
...
- The change introduces new user-facing mechanics that affect the
 product's MDA aesthetic feature tradeoff
...
using `/design-system` to author a full PRD for this."

---

## Recommended Next Steps

- Run `/story-readiness [story-path]` to validate the story before implementation begins — reference this spec in the story's PRD Reference field
- Run `/dev-story [story-path]` to implement once the story passes readiness checks
- If the change is larger than expected, run `/design-system [system-name]` to author a full PRD instead
