...
description: "Validate that a story file is implementation-ready. Checks for embedded PRD requirements, ADR references, framework notes, clear acceptance criteria, and no open design questions. Produces READY / NEEDS WORK / BLOCKED verdict with specific gaps. Use when user says 'is this story ready', 'can I start on this story', 'is story X ready to implement'."
...
- **Specific path** (e.g., `/story-readiness production/epics/billing/story-001-apply-discount.md`):
...
- `design/prd/systems-index.md` — to know which systems have approved PRDs
...
- [ ] **PRD requirement referenced**: The story includes a `design/prd/` path
 ...
 that PRD — not just the PRD filename. A link to the document without tracing
 to a specific requirement does not pass.
- [ ] **Requirement is self-contained**: The acceptance criteria in the story
 are understandable without opening the PRD. A developer should not need to
...
- [ ] **Manifest version is current**: If the story has a `Manifest Version:` date
 ...
 does not exist.
- [ ] **Framework notes present**: For any post-cutoff framework API this story
 is likely to touch, implementation notes or a verification requirement are
 included. If the story clearly does not touch framework APIs (e.g., it is a
 pure data/config change), "N/A — no framework API involved" is acceptable.
...
- [ ] **Performance budget noted if applicable**: If this story touches any
 part of the core user flow, rendering, or physics, a performance budget or
...
- If a story file does not exist at all: "This story file is missing entirely.
 Run `/create-epics [layer]` then `/create-stories [epic-slug]` to generate stories from the PRD and ADR."
...
- If a story's scope has grown beyond its original sizing: "This story appears
 to have expanded in scope. Consider splitting it or escalating to the engineering manager
