...
- **Feature name** → read `design/prd/[feature].md` or matching file in `design/`
...
4. **Flag**: [Items that need a decision from engineering manager/product-director]
```
...
| >50% | **FAIL** | Out of Control — stop, re-plan, escalate to engineering manager |
...
- **FAIL** → recommend escalating to engineering manager. Reference `/sprint-plan update` for re-planning or `/estimate` to re-baseline timeline.
