...
**Required Artifacts:**
- [ ] `design/prd/product-concept.md` exists and has content
- [ ] Product pillars defined (in concept doc or `design/prd/product-pillars.md`)
- [ ] Visual Identity Anchor section exists in `design/prd/product-concept.md` (from brainstorm Phase 4 design-director output)
...
**Required Artifacts:**
- [ ] Systems index exists at `design/prd/systems-index.md` with at least MVP systems enumerated
- [ ] All MVP-tier PRDs exist in `design/prd/` and individually pass `/design-review`
- [ ] A cross-PRD review report exists in `design/prd/` (from `/review-all-prds`)

**Quality Checks:**
- [ ] All MVP PRDs pass individual design review (8 required sections, no MAJOR REVISION NEEDED verdict)
- [ ] `/review-all-prds` verdict is not FAIL (cross-PRD consistency and design theory checks pass)
- [ ] All cross-PRD consistency issues flagged by `/review-all-prds` are resolved or explicitly accepted
- [ ] System dependencies are mapped in the systems index and are bidirectionally consistent
- [ ] MVP priority tier is defined
- [ ] No stale PRD references flagged (older PRDs updated to reflect decisions made in later PRDs)
...
**Required Artifacts:**
- [ ] Framework chosen (CLAUDE.md Technology Stack is not `[CHOOSE]`)
- [ ] Technical preferences configured (`.claude/docs/technical-preferences.md` populated)
- [ ] Art bible exists at `design/art/design-system-bible.md` with at least Sections 1–4 (Visual Identity Foundation)
...
- [ ] Framework reference docs exist in `docs/framework-reference/[framework]/`
...
- [ ] At least one screen's UX spec started (often the dashboard or onboarding is designed during Technical Setup)
- [ ] All ADRs have an **Framework Compatibility section** with framework version stamped
- [ ] All ADRs have a **PRD Requirements Addressed section** with explicit PRD linkage
- [ ] No ADR references APIs listed in `docs/framework-reference/[framework]/deprecated-apis.md`
- [ ] All HIGH RISK framework domains (per VERSION.md) have been explicitly addressed
...
**Framework Validation** (read `docs/framework-reference/[framework]/VERSION.md` first):
- [ ] ADRs that touch post-cutoff framework APIs are flagged with Knowledge Risk: HIGH/MEDIUM
- [ ] `/architecture-review` framework audit shows no deprecated API usage
- [ ] All ADRs agree on the same framework version (no stale version references)
...
- [ ] Design system bible is complete (all required sections) and the design-director sign-off verdict is recorded in `design/prd/design-system-bible.md`
...
- [ ] Control manifest exists at `docs/architecture/control-manifest.md`
 (generated by `/create-permissions-manifest` from Accepted ADRs)
...
- [ ] Beta build has been usability-tested with at least 3 sessions (internal OK)
- [ ] Vertical Slice usability test report exists at `production/usability tests/` or equivalent
- [ ] UX specs exist for key screens: primary dashboard, key list / detail screens, settings (at `design/ux/`)
- [ ] Dashboard design document exists at `design/ux/dashboard.md` (if product has a dashboard)
- [ ] All key screen UX specs have passed `/ux-review` (verdict APPROVED or NEEDS REVISION accepted)

**Quality Checks:**
- [ ] **Core loop fun is validated** — usability test data confirms the central mechanic is enjoyable, not just functional. Explicitly check the Vertical Slice usability test report.
- [ ] UX specs cover all UI Requirements sections from MVP-tier PRDs
...
- [ ] Sprint plan references real story file paths from `production/epics/`
 (not just PRDs — stories must embed PRD req ID + ADR reference)
- [ ] **Vertical Slice is COMPLETE**, not just scoped — the build demonstrates the full core user flow end-to-end. At least one complete [start → challenge → resolution] cycle works.
- [ ] Architecture document has no unresolved open questions in Foundation or Core layers
- [ ] All ADRs have Framework Compatibility sections stamped with the framework version
- [ ] All ADRs have ADR Dependencies sections (even if all fields are "None")
- [ ] Manual validation confirms PRDs + architecture + epics are coherent
 (run `/review-all-prds` and `/architecture-review` if not done recently)
- [ ] **Core fantasy is delivered** — at least one usability tester independently described an experience that matches the User Fantasy section of the core system PRDs (without being prompted).

**Vertical Slice Validation** (FAIL if any item is NO):
- [ ] A human has played through the core user flow without developer guidance
...
> production failure in web/SaaS development (per GDC postmortem data from 155 projects).
...
- [ ] All core mechanics from PRD are implemented (cross-reference `design/prd/` with `src/`)
- [ ] Main user flow path is playable end-to-end
...
- [ ] At least 3 distinct usability test sessions documented in `production/usability tests/`
- [ ] Usability Test reports cover: new user experience, mid-product systems, and learning curve
- [ ] Fun hypothesis from Product Concept has been explicitly validated or revised
...
- [ ] Usability Test findings have been reviewed and critical fun issues addressed (not just documented)
- [ ] No "confusion loops" identified — no point in the product where >50% of usability-test participants got stuck without knowing why
- [ ] Activation milestones and pacing match the activation plan (if one exists at `design/growth-ops/activation/`)
...
- [ ] Feature Tradeoff data has been reviewed (`/tradeoff-check` run)
...
Systems Design → Technical Setup, pull entries in Economy, Core feature interaction, or any PRD domain;
if checking Technical Setup → Pre-Production, pull entries in Architecture, Framework).
...
**Systems Design → Technical Setup gate — cross-PRD review check**:
Use `Glob('design/prd/prd-cross-review-*.md')` to find the `/review-all-prds` report.
If no file matches, mark the "cross-PRD review report exists" artifact as **FAIL** and
surface it prominently: "No `/review-all-prds` report found in `design/prd/`. Run
`/review-all-prds` before advancing to Technical Setup."
If a file is found, read it and check the verdict line: a FAIL verdict means the
cross-PRD consistency check failed and must be resolved before advancing.
...
- Compare `design/prd/` documents against `src/` implementations
...
- "I can't automatically verify that the core workflow feels right. Has it been usability-tested?"
- "No usability test report found. Has informal testing been done?"
...
1. **`product-director`** — gate **CD-PHASE-GATE** (`.claude/docs/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`.claude/docs/director-gates.md`)
3. **`engineering manager`** — gate **PR-PHASE-GATE** (`.claude/docs/director-gates.md`)
4. **`design-director`** — gate **AD-PHASE-GATE** (`.claude/docs/director-gates.md`)
...
Product Director: [READY / CONCERNS / NOT READY]
 [feedback]

Technical Director: [READY / CONCERNS / NOT READY]
 [feedback]

Engineering Manager: [READY / CONCERNS / NOT READY]
 [feedback]

Design Director: [READY / CONCERNS / NOT READY]
 [feedback]
```
...
- [x] design/prd/product-concept.md — exists, 2.4KB
...
- [?] Core workflow usability-tested — MANUAL CHECK NEEDED
...
[A] Run /create-architecture — produce your master architecture server-actions and ADR work plan (recommended next step)
[B] Design more PRDs first — return here when all MVP systems are complete
[C] Stop here for this session
```

> **Note for systems-design PASS**: `/create-architecture` is the required next step before writing any ADRs. It produces the master architecture document and a prioritized list of ADRs to write. Running `/architecture-decision` without this step means writing ADRs without a server-actions — skip it at your own risk.
...
- **No design system bible?** → `/design-system-bible` to create the visual identity specification
- **Art bible exists but no asset specs?** → `/asset-spec system:[name]` to generate per-asset visual specs and generation prompts from approved PRDs
...
- **Small design change needed?** → `/quick-design` for changes under ~4 hours (bypasses full PRD pipeline)
...
- **Cross-PRD consistency issues?** → fix flagged PRDs, then re-run `/review-all-prds`
- **No test framework?** → `/test-setup` to scaffold the framework for your framework
...
- **No master architecture doc?** → `/create-architecture` for the full server-actions
- **ADRs missing framework compatibility sections?** → Re-run `/architecture-decision`
 or manually add Framework Compatibility sections to existing ADRs
- **Missing control manifest?** → `/create-permissions-manifest` (requires Accepted ADRs)
...
- **Tests failing?** → delegate to `lead-engineer` or `qa-engineer`
- **No usability test data?** → `/usability-test-report`
- **Less than 3 usability test sessions?** → Run more usability tests before advancing. Use `/usability-test-report` to structure findings.
- **No activation plan?** → Consider running `/team-growth` to define one before polish
- **No user journey document?** → create `design/user-journey.md` using the user journey template
