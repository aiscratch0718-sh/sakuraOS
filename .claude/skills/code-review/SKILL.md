...
Read `.claude/docs/technical-preferences.md`, section `## Framework Specialists`. Note:

- The **Primary** specialist (used for architecture and broad framework concerns)
- The **Language/Code Specialist** (used when reviewing the project's primary language files)
- The **Stylesheet Specialist** (used when reviewing stylesheet files)
- The **UI Specialist** (used when reviewing UI code)

If the section reads `[TO BE CONFIGURED]`, no framework is pinned — skip framework specialist steps.
...
Identify the system category (framework, user flow, AI, networking, UI, tools) and evaluate:

- [ ] Public methods and classes have doc comments
- [ ] Cyclomatic complexity under 10 per method
- [ ] No method exceeds 40 lines (excluding data declarations)
- [ ] Dependencies are injected (no static singletons for product state)
...
**Architecture:**
- [ ] Correct dependency direction (framework <- user flow, not reverse)
- [ ] No circular dependencies between modules
- [ ] Proper layer separation (UI does not own product state)
...
## Phase 6: Product-Specific Concerns
...
If a framework is configured, determine which specialist applies to each file and spawn in parallel:

- Primary language files (`.gd`, `.cs`, `.cpp`) → Language/Code Specialist
- Stylesheet files (`.gdshader`, `.hlsl`, stylesheet graph) → Stylesheet Specialist
- UI screen/widget code → UI Specialist
- Cross-cutting or unclear → Primary Specialist

Also spawn the **Primary Specialist** for any file touching framework architecture (scene structure, node hierarchy, lifecycle hooks).

### QA Testability Review

For Logic and Integration stories, also spawn `qa-engineer` via Task in parallel with the framework specialists. Pass:
...
### Framework Specialist Findings: [N/A — no framework configured / CLEAN / ISSUES FOUND]
[Findings from framework specialist(s), or "No framework configured." if skipped]
...
[List web/SaaS development specific issues]
