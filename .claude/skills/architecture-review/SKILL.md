...
description: "Validates completeness and consistency of the project architecture against all PRDs. Builds a traceability matrix mapping every PRD technical requirement to ADRs, identifies coverage gaps, detects cross-ADR conflicts, verifies framework compatibility consistency across all decisions, and produces a PASS/CONCERNS/FAIL verdict. The architecture equivalent of /design-review."
argument-hint: "[focus: full | coverage | consistency | framework | single-prd path/to/prd.md]"
user-invocable: true
...
covers all product design requirements, is internally consistent, and correctly targets
the project's pinned framework version. It is the quality gate between Technical Setup
and Pre-Production.
...
- **`framework`**: Framework compatibility audit only
...
- **`rtm`**: Requirements Traceability Matrix — extends the standard matrix
 ...
 PRD requirement → ADR → Story → Test chain. Use in Production phase when
...
only those ADRs. Skip full-reading unrelated PRDs entirely.

For `framework` mode: only full-read ADRs — PRDs are not needed for framework checks.
...
### Design Documents
- All in-scope PRDs in `design/prd/` — read every file completely
- `design/prd/systems-index.md` — the authoritative list of systems
...
### Framework Reference
- `docs/framework-reference/[framework]/VERSION.md`
- `docs/framework-reference/[framework]/breaking-changes.md`
- `docs/framework-reference/[framework]/deprecated-apis.md`
- All files in `docs/framework-reference/[framework]/modules/`

### Project Standards
- `.claude/docs/technical-preferences.md`

Report a count: "Loaded [N] PRDs, [M] ADRs, framework: [name + version]."

**Also read `docs/consistency-failures.md`** if it exists. Extract entries with
Domain matching the systems under review (Architecture, Framework, or any PRD domain
...
| **Framework capability** | "Server Components for the dashboard tree" → SSR / streaming ADR |
| **Cross-feature communication** | "Billing event triggers email + audit log + analytics" → event-bus / domain-event ADR |
| **State persistence** | "Imported records survive restarts and partial failures" → idempotent-write / outbox-pattern ADR |
| **Threading / async** | "Long-running exports run in a background queue" → queue / worker ADR |
| **Surface requirements** | "Supports desktop and mobile web" → responsive / breakpoint ADR |
...
Technical Requirements:
 TR-[PRD]-001: [requirement text] → Domain: [Physics/Rendering/etc]
 TR-[PRD]-002: [requirement text] → Domain: [...]
```
...
1. Read every ADR's "PRD Requirements Addressed" section
2. Check if it explicitly references the requirement or its PRD
...
| Requirement ID | PRD | System | Requirement | ADR Coverage | Status |
|---------------|-----|--------|-------------|--------------|--------|
| TR-BILL-001 | billing.md | Billing | Discount calc accuracy on multi-line invoices | ADR-0003 | ✅ |
| TR-BILL-002 | billing.md | Billing | Idempotent payment-failure handling | — | ❌ GAP |
| TR-AUDIT-001 | audit-log.md | Audit | Audit-write in same transaction as state change | ADR-0005 | ✅ |
```
...
|-------|-----|-------------|-----|-------|-----------|-------------|
| TR-BILL-001 | billing.md | Discount calc accuracy | ADR-0003 | story-001-discount.md | tests/unit/billing/discount.test.ts | COVERED |
| TR-BILL-002 | billing.md | Idempotent payment failure | — | story-002-failure.md | — | NONE (pending impl) |
| TR-AUDIT-001 | audit-log.md | Same-transaction audit | ADR-0005 | — | — | NO STORY |
```
...
- **State management conflict**: Two ADRs define authority over the same product state
 (e.g. both Core feature interaction ADR and Character ADR claim to own the health value)
...
## Phase 5: Framework Compatibility Cross-Check

Across all ADRs, check for framework consistency:

### Version Consistency
- Do all ADRs that mention a framework version agree on the same version?
- If any ADR was written for an older framework version, flag it as potentially stale
...
### Missing Framework Compatibility Sections
- List all ADRs that are missing the Framework Compatibility section entirely
- These are blind spots — their framework assumptions are unknown

Output format:
```
### Framework Audit Results
Framework: [name + version]
ADRs with Framework Compatibility section: X / Y total
...
After completing the framework audit above, spawn the **primary framework specialist** via Task for a domain-expert second opinion:
- Read `.claude/docs/technical-preferences.md` `Framework Specialists` section to get the primary specialist
- If no framework is configured, skip this consultation
- Spawn `subagent_type: [primary specialist]` with: all ADRs that contain framework-specific decisions or `Post-Cutoff APIs Used` fields, the framework reference docs, and the Phase 5 audit findings. Ask them to:
 1. Confirm or challenge each audit finding — specialists may know of framework nuances not captured in the reference docs
 2. Identify framework-specific anti-patterns in the ADRs that the audit may have missed (e.g., using the wrong Next.js node type, React component coupling, NestJS subsystem misuse)
 3. Flag ADRs that make assumptions about framework behaviour that differ from the actual pinned version

Incorporate additional findings under `### Framework Specialist Findings` in the Phase 5 output. These feed into the final verdict — specialist-identified issues carry the same weight as audit-identified issues.

---

## Phase 5b: Design Revision Flags (Architecture → PRD Feedback)

For each **HIGH RISK framework finding** from Phase 5, check whether any PRD makes an
assumption that the verified framework reality contradicts.
...
1. **Post-cutoff API behaviour differs from training-data assumptions**: If an ADR
 ...
 check all PRDs that reference the related system. Look for design rules written
 around the old (assumed) behaviour.

2. **Known framework limitations in ADRs**: If an ADR records a known framework limitation
 (e.g. "Jolt ignores HingeJoint3D damp", "D3D12 is now the default backend"), check
 PRDs that design mechanics around the affected feature.

3. **Deprecated API conflicts**: If Phase 5 flagged a deprecated API used in an ADR,
 check whether any PRD contains mechanics that assume the deprecated API's behaviour.

For each conflict found, record it in the PRD Revision Flags table:

```
### PRD Revision Flags (Architecture → Design Feedback)
These PRD assumptions conflict with verified framework behaviour or accepted ADRs.
The PRD should be revised before its system enters implementation.

| PRD | Assumption | Reality (from ADR/framework-reference) | Action |
|-----|-----------|--------------------------------------|--------|
| billing.md | "Use synchronous webhook callback to mark invoice paid" | Stripe webhooks must be processed async — ADR-0003 | Revise PRD |
```

If no revision flags are found, write: "No PRD revision flags — all PRD assumptions
are consistent with verified framework behaviour."

Ask: "Should I flag these PRDs for revision in the systems index?"
...
If `docs/architecture/architecture.md` exists, validate it against PRDs:

- Does every system from `systems-index.md` appear in the architecture layers?
- Does the data flow section cover all cross-system communication defined in PRDs?
- Do the API boundaries support all integration requirements from PRDs?
- Are there systems in the architecture doc that have no corresponding PRD
...
### PRD Revision Flags
[PRD assumptions that conflict with verified framework behaviour — from Phase 5b]
[Or: "None — all PRD assumptions consistent with verified framework behaviour"]

### Framework Compatibility Issues
[List all framework issues from Phase 5]
...
PASS: All requirements covered, no conflicts, framework consistent
...
> Coverage: [N]% full chain complete (PRD → ADR → Story → Test)
...
| TR-ID | PRD | Requirement | ADR | Story | Test File | Status |
|-------|-----|-------------|-----|-------|-----------|--------|
...
[Requirements whose PRD was changed after the ADR was written]
```
