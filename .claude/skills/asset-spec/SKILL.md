---
name: asset-spec
description: "Generate per-asset visual specifications and AI generation prompts from PRDs, level docs, or character profiles. Produces structured spec files and updates the master asset manifest. Run after design system bible and PRD/screen design are approved, before production begins."
...
- If no manifest: fail with:
 ...
 > Run after your design system bible and PRDs are approved."
...
**Mode behavior:**
- `full` (default): spawn both `design-director` and `design-systems-engineer` in parallel
- `lean`: spawn `design-director` only — faster, skips technical constraint pass
- `solo`: no agent spawning — main session writes specs from design system bible rules alone. Use for simple asset categories or when speed matters more than depth.
...
### Required reads:
- **Art bible**: Read `design/art/design-system-bible.md` — fail if missing:
 > "No design system bible found. Run `/design-system-bible` first — asset specs are anchored to the design system bible's visual rules and asset standards."
...
- **system**: Read `design/prd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
 > "The Visual/Audio section of `design/prd/[target-name].md` is empty. Either run `/design-system [target-name]` to complete the PRD, or describe the visual needs manually."
 Use `AskUserQuestion`: `[A] Describe needs manually` / `[B] Stop — complete the PRD first`
- **level**: Read `design/levels/[target-name].md`. Extract art requirements, asset list, VFX needs, and the design-director's production concept specs from Step 4.
...
- **Icons** — UI icons, status badges, navigation icons
- **Illustrations** — empty-state illustrations, marketing illustrations, onboarding artwork
- **Marketing imagery** — hero images, feature screenshots, OG / social cards
- **UI components** — dashboard widget thumbnails, custom backgrounds
- **Fonts** — custom or licensed font files (if not Google Fonts / system stack)
- **Brand audio** (if applicable) — notification sound, brand jingle *(note: audio specs are descriptions only — no generation prompts)*
...
**`design-director`** via Task:
- Provide: full asset list from Phase 2, design system bible Visual Identity Statement, Color System, Shape Language, the source doc's visual requirements, and any reference products/art mentioned in the design system bible Section 9
- Ask: "For each asset in this list, produce: (1) a 2–3 sentence visual description anchored to the design system bible's shape language and color system — be specific enough that two different artists would produce consistent results; (2) a generation prompt ready for use with AI image tools (Midjourney/Stable Diffusion style — include style keywords, composition, color palette anchors, negative prompts); (3) which design system bible rules directly govern this asset (cite by section). For audio assets, describe the sonic character instead of a generation prompt."

**`design-systems-engineer`** via Task:
- Provide: full asset list, design system bible Asset Standards (Section 8), technical-preferences.md performance budgets, framework name and version
- Ask: "For each asset in this list, specify: (1) exact dimensions or polycount (match the design system bible Asset Standards tiers — do not invent new sizes); (2) file format and export settings; (3) naming convention (from technical-preferences.md); (4) any framework-specific constraints this asset type must respect; (5) LOD requirements if applicable. Flag any asset type where the design system bible's preferred standard conflicts with the framework's constraints."

### Lean mode — spawn design-director only (skip design-systems-engineer).

### Solo mode — skip both. Derive specs from design system bible rules alone, noting that technical constraints were not validated.

**Collect both responses before Phase 4.** If any conflict exists between design-director and design-systems-engineer (e.g., design-director specifies 4K textures but design-systems-engineer flags the framework budget requires 512px), surface it explicitly — do NOT silently resolve.
...
| Texture Res | [e.g. 512px — matches Design System Bible §8 Tier 2] |

**Visual Description:**
[2–3 sentences. Specific enough for two artists to produce consistent results.]

**Design System Bible Anchors:**
...
> **Source**: [path to source PRD/level/character doc]
> **Design System Bible**: design/art/design-system-bible.md
> **Generated**: [date]
...
> "ASSET-012 (Generic Hit Spark) already specced for Core feature interaction system. Reusing for Tower Defense — adding tower-defense to referenced-by."
...
2. In `lean` mode or if `design-systems-engineer` blocks: proceed with design-director output only — note that technical constraints were not validated
3. In `solo` mode or if `design-director` blocks: derive descriptions from design system bible rules — flag as "Art director not consulted — verify against design system bible before production"
...
- Always anchor specs to the design system bible — a spec that contradicts the design system bible is wrong
