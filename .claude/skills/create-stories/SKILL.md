---
name: create-stories
description: "Break a single epic into implementable story files. Reads the epic, its PRD, governing ADRs, and control manifest. Each story embeds its PRD requirement TR-ID, ADR guidance, acceptance criteria, story type, and test evidence path. Run after /create-epics for each epic."
...
focused session, self-contained, and fully traceable to a PRD requirement and
...
- `/create-stories [epic-slug]` — e.g. `/create-stories billing`
- `/create-stories production/epics/billing/EPIC.md` — full path also accepted
...
- `production/epics/[epic-slug]/EPIC.md` — epic overview, governing ADRs, PRD requirements table
- The epic's PRD (`design/prd/[filename].md`) — read all 8 sections, especially Acceptance Criteria, Formulas, and Edge Cases
- All governing ADRs listed in the epic — read the Decision, Implementation Guidelines, Framework Compatibility, and Framework Notes sections
...
Report: "Loaded epic [name], PRD [filename], [N] governing ADRs (all confirmed present), control manifest v[date]."
...
|---|---|
| **Logic** | Formulas, numerical thresholds, state transitions, validation rules, billing rules, calculations |
| **Integration** | Two or more systems interacting, domain events crossing feature boundaries, database round-trips, queue jobs, webhook ingress / egress |
| **Visual / Feel** | Animation behaviour, motion, "feels responsive", perceived timing, focus order |
| **UI** | Navigation, dashboard widgets, list / detail screens, forms, dialogs, toasts, tooltips |
| **Config / Data** | Entitlement / pricing / feature-flag values, seed data — no new code logic |
...
- **Framework risk**: from the ADR's Knowledge Risk field
...
Pass: the full story list with acceptance criteria, story types, and TR-IDs; the epic's PRD acceptance criteria for reference.
...
**Framework Notes**: [from ADR Framework Compatibility section — post-cutoff APIs, verification required]
...
*From PRD `design/prd/[filename].md`, scoped to this story:*

- [ ] [criterion 1 — directly from PRD]
- [ ] [criterion 2]
...
- Integration: `tests/integration/[system]/[story-slug]_test.[ext]` OR usability test doc
...
5. **No invention** — acceptance criteria come from PRDs, implementation notes from ADRs, rules from the manifest
