---
name: team-growth
description: "Orchestrate the growth-ops team for post-launch content planning: coordinates growth-engineer, business-analyst, analytics-engineer, customer-success-manager, content-writer, and content-director to design and plan a season, event, or live content update."
...
> "Usage: `/team-growth [season name or event description]` — Provide the name or description of the season or live event to plan."
Then stop immediately without spawning any subagents or reading any files.

When this skill is invoked with a valid argument, orchestrate the growth-ops team through a structured planning pipeline.
...
## Team Composition
- **growth-engineer** — Season structure, event cadence, retention mechanics, battle pass
- **business-analyst** — Live economy feature tradeoff, store rotation, currency pricing, pity timers
- **analytics-engineer** — Success metrics, A/B test design, event tracking, dashboard specs
- **customer-success-manager** — User-facing announcements, event descriptions, seasonal messaging
- **content-director** — Seasonal narrative theme, story arc, world event framing
- **content-writer** — Event descriptions, reward item names, seasonal flavor text, announcement copy

## How to Delegate

Use the Task tool to spawn each team member as a subagent:
- `subagent_type: growth-engineer` — Season/event structure and retention mechanics
- `subagent_type: business-analyst` — Live economy feature tradeoff and reward pricing
- `subagent_type: analytics-engineer` — Success metrics, A/B tests, event instrumentation
- `subagent_type: customer-success-manager` — User-facing communication and messaging
- `subagent_type: content-director` — Seasonal theme and narrative framing
- `subagent_type: content-writer` — All user-facing text: event descriptions, item names, copy

Always provide full context in each agent's prompt (product concept path, existing season docs, ethics policy path, current economy state). Launch independent agents in parallel where the pipeline allows it (Phases 3 and 4 can run simultaneously).

## Pipeline

### Phase 1: Season/Event Scoping
Delegate to **growth-engineer**:
...
- Define the retention hook: what brings users back daily/weekly during this season
...
Delegate to **content-director**:
- Read the season brief from Phase 1
- Design the seasonal narrative theme: how does this event connect to the product world?
- Define the central story hook users will discover during the event
...
- Read the season brief and existing economy rules from `design/growth-ops/economy-rules.md`
...
- **content-director** (if needed): Write any in-app narrative text (cutscene scripts, system actor dialogue, world event descriptions) for the season
- **content-writer**: Write all user-facing text — event names, reward item descriptions, challenge objective text, seasonal flavor text
- Both should read the narrative framing doc from Phase 2

### Phase 6: User Communication Plan
Delegate to **customer-success-manager**:
...
- Draft known-issues section placeholder for day-1 release notes
...
- **Ethics review**: check the Phase 3 economy design against `design/growth-ops/ethics-policy.md`
 - If the file does not exist: flag "ETHICS REVIEW SKIPPED: `design/growth-ops/ethics-policy.md` not found. Economy design was not reviewed against an ethics policy. Recommend creating one before production begins." Include this flag in the season design output document. Add to next steps: create `design/growth-ops/ethics-policy.md`.
 - If the file exists and a violation is found: flag "ETHICS FLAG: [element] in Phase 3 economy design violates [policy rule]. Approval is blocked until this is resolved." Do NOT issue a COMPLETE verdict or write output documents. Use `AskUserQuestion` with options: revise economy design / override with documented rationale / cancel. If user chooses to revise: re-spawn business-analyst to produce a corrected design, then return to Phase 7 review.
...
All documents save to `design/growth-ops/`:
