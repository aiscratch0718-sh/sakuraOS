...
**Required:**
- Read `design/prd/product-concept.md` — **fail with a clear message if missing**:
 > "No product concept found at `design/prd/product-concept.md`. Run `/brainstorm` first
 > to create one, then come back to decompose it into systems."

**Optional (read if they exist):**
- Read `design/prd/product-pillars.md` — pillars constrain priority and scope
- Read `design/prd/systems-index.md` — if exists, **resume** from where it left off
 (update, don't recreate from scratch)
- Glob `design/prd/*.md` — check which system PRDs already exist
...
- "Authentication" implies: signup, signin, password reset, session
  management, RBAC, audit log of auth events
- "Multi-tenancy" implies: tenant_id propagation, RLS or repository
  scoping, per-tenant configuration, tenant deletion lifecycle
- "Billing" implies: pricing tiers, entitlement checks, usage metering,
  invoice generation, payment-failure handling, dunning
- "Dashboard" implies: widget framework, refresh model, drill-down
  routing, per-role widget visibility, performance budget enforcement
- "Notifications" implies: dispatcher, channel adapters (email / in-app /
  push), idempotency, frequency caps, quiet hours, customer preferences
- "Search" implies: indexer, query parser, ranking, filters, pagination,
  per-tenant scoping, reindex on schema change
- "Reporting" implies: report definitions, scheduled runs, export
  pipeline (CSV / PDF / API), per-role visibility, large-result handling
- "Realtime collaboration" implies: WebSocket / SSE channel, presence
  tracking, conflict resolution (CRDT or last-write-wins), reconnection
...
- **UI dependencies**: Every user flow system has a corresponding UI system that
 depends on it (but UI is designed after the user flow system)
...
4. **Presentation**: UI and feedback systems that wrap user flow systems
...
- **Alpha**: All remaining user flow systems
...
Explain reasoning in conversation: "I placed [system] in MVP because the core user flow
requires it — without [system], the 30-second loop can't function."

**"Why" column guidance**: When explaining why each system was placed in a priority tier, mix technical necessity with user-experience reasoning. Do not use purely technical justifications like "Core feature interaction needs damage math" — connect to user experience where relevant. Examples of good "Why" entries:
- "Required for the core user flow — without it, placement decisions have no consequence (Pillar 2: Placement is the Puzzle)"
- "Ballista's punch-through identity is established here — this stat definition is what makes it feel different from Archer"
- "Foundation for all economy decisions — users must understand upgrade costs to make meaningful placement choices"

Pure technical necessity ("X depends on Y") is insufficient alone when the system directly shapes user experience.
...
**After priorities are approved, spawn `engineering manager` via Task using gate PR-SCOPE (`.claude/docs/director-gates.md`) before writing the index.**
...
Ask: "May I write the systems index to `design/prd/systems-index.md`?"
...
**After the systems index is written, spawn `product-director` via Task using gate CD-SYSTEMS (`.claude/docs/director-gates.md`).**

Pass: systems index path, product pillars and core fantasy (from `design/prd/product-concept.md`), MVP priority tier system list.

Present the assessment. If REJECT, revise the system set with the user before PRD authoring begins. If CONCERNS, record them in the systems index as a `> **Product Director Note**` at the top of the relevant tier section.
...
- File: design/prd/systems-index.md
- Next: Design individual system PRDs

**Verdict: COMPLETE** — systems index written to `design/prd/systems-index.md`.
...
- "Systems index is written. What would you like to do next?"
 - [A] Start designing PRDs — run `/design-system [first-system-in-order]`
 - [B] Ask a director to review the index first — ask `product-director` or `technical-director` to validate the system set before committing to 10+ PRD sessions
 - [C] Stop here for this session

**The director review option ([B]) is worth highlighting**: having a Product Director or Technical Director review the completed systems index before starting PRD authoring catches scope issues, missing systems, and boundary problems before they're locked in across many documents. It is optional but recommended for new projects.

After any individual PRD is completed:
- "Run `/design-review design/prd/[system].md` in a fresh session to validate quality"
- "Run `/gate-check systems-design` when all MVP PRDs are complete"
...
> `design/prd/systems-index.md`. Open a fresh Claude Code session to continue
> designing individual PRDs — run `/map-systems next` to pick up where you left off.

---

## Recommended Next Steps

- Run `/design-system [first-system-in-order]` to author the first PRD (use design order from the index)
- Run `/map-systems next` to always pick the highest-priority undesigned system automatically
- Run `/design-review design/prd/[system].md` in a fresh session after each PRD is authored
