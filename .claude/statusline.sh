#!/usr/bin/env bash
# Claude Code Web Studio — Status Line
...
if [ -z "$stage" ]; then
  concept_file="$cwd/design/prd/product-concept.md"
  ...
  # Check if framework is configured (not placeholder)
  if [ -f "$tech_prefs" ]; then
    engine_line=$(grep -m1 '^\*\*Framework\*\*:' "$tech_prefs" 2>/dev/null || true)
  ...
  if [ -d "$cwd/src" ]; then
    src_count=$(find "$cwd/src" -type f \( -name "*.ts" -o -name "*.cs" -o -name "*.cpp" -o -name "*.h" -o -name "*.py" -o -name "*.rs" -o -name "*.lua" -o -name "*.tscn" -o -name "*.tres" \) 2>/dev/null | wc -l | tr -d ' ')
  fi
