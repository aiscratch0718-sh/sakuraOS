...
for file in $MERGED_ASSETS; do
 ...
 # Audio: max 10MB for music, 512KB for UI sound cues
 if [[ "$file" == assets/audio/ui-sound-cue* ]] && [ "$size" -gt 524288 ]; then
 WARNINGS="$WARNINGS\nBUDGET: $file -- ${size} bytes exceeds 512KB UI sound cues budget"
 fi
 fi
done
...
1. For naming violations: fix manually or invoke `design-director` for guidance
2. For size violations: invoke `design-systems-engineer` for optimization advice
