...
if [ -n "$CODE_FILES" ]; then
 ...
 # Check for hardcoded magic numbers in user flow code
 if [[ "$file" == src/user flow/* ]]; then
 # Look for numeric literals that are likely feature tradeoff values
 # Adjust the pattern for your language
 if grep -nE '(damage|health|speed|rate|chance|cost|duration)[[:space:]]*[:=][[:space:]]*[0-9]+' "$file"; then
 echo "WARNING: $file may contain hardcoded user flow values. Use data files."
 ...
 # For TypeScript: gdlint "$file" || EXIT_CODE=1
 # For C#: dotnet format --check "$file" || EXIT_CODE=1
 # For Node.js: clang-format --dry-run -Werror "$file" || EXIT_CODE=1
 done
...
1. For style violations: auto-fix with your formatter or invoke `lead-engineer`
2. For hardcoded values: invoke `feature-engineer` to externalize the values
3. For test failures: invoke `qa-engineer` to diagnose and `feature-engineer` to fix
