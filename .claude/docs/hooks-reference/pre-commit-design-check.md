...
Ensures design documents and product data files maintain consistency and
...
```bash
#!/bin/bash
# Pre-commit hook: Design document and product data validation
...
1. For missing design sections: invoke the `product-manager` agent to complete
 the document
2. For invalid JSON: invoke the `devx-engineer` agent or fix manually
