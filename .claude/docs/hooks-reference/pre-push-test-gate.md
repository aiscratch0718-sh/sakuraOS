...
if [ "$FULL_GATE" = true ]; then
 ...
 # python -m pytest tests/usability test/smoke/ -x || exit 1
...
1. Build failure: invoke `lead-engineer` to diagnose
2. Unit test failure: invoke `qa-engineer` to identify the failing test and
 `feature-engineer` or relevant programmer to fix
3. Performance regression: invoke `performance-engineer` to analyze
