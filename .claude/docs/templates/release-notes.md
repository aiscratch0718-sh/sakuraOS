# Release Notes: [Product Title] v[Version]

*Released: [Date]*

---

## Headline

> [One compelling sentence summarizing the most exciting part of this release.
> This is what appears in store update notifications and social media.]

---

## What's New

### [Major Feature 1 Name]
[2-4 sentences describing the feature. Focus on what users can DO, not
how it works internally. Include a screenshot or GIF reference if applicable.]

### [Major Feature 2 Name]
[Description]

---

## User-facing behavior Changes

### Feature Tradeoff Adjustments

| Target | Change | Before | After | Context |
| ---- | ---- | ---- | ---- | ---- |
| [Tool/Ability/Item] | [What changed] | [Old value] | [New value] | [Why — 1 sentence] |
| | | | | |

### Mechanic Changes
- **[Change Name]**: [What changed and how it affects user flow. Be specific
 about what users will experience differently.]

---

## Quality of Life

- [Improvement 1 — describe the user benefit, not the technical change]
- [Improvement 2]
- [Improvement 3]

---

## Bug Fixes

### Critical Fixes
- Fixed [describe what users experienced, e.g., "a crash when loading
 saved products from version 1.0"]

### User-facing behavior Fixes
- Fixed [describe the incorrect behavior and the correct behavior now]

### UI Fixes
- Fixed [description]

### Audio Fixes
- Fixed [description]

### Platform-Specific Fixes
- **[Platform]**: Fixed [description]

---

## Performance Improvements

- [Improvement users will notice, e.g., "Reduced load times by approximately
 30% on all platforms"]
- [Improvement]

---

## Known Issues

We are aware of the following issues and are working on fixes for a future update:

- **[Issue]**: [Brief description and workaround if available]
- **[Issue]**: [Description]

---

## Coming Next

[Optional — tease upcoming content to build anticipation. Keep it vague enough
to not create binding commitments.]

> [1-2 sentences about what the team is working on next]

---

## Thank You

[Brief message thanking customers. Reference specific customer feedback
that influenced changes in this release if applicable.]

---

*For the full changelog with technical details, see the [developer changelog](link).*
*Report bugs: [link to support form]*
*Stay updated: [link to status page / blog / mailing list]*
