...
- **Product state at start**: [e.g., "fresh tenant, default RBAC roles, single admin user, no records"]
- **Browser / environment**: [e.g., "Chrome 124 on macOS 14, 1440px viewport, fast 3G throttling off"]
- **Performance during test**: [e.g., "TTFB 220ms, LCP 1.2s — within budget"]
- **Any special setup required**: [e.g., "feature flag X enabled via admin UI to trigger this state"]
