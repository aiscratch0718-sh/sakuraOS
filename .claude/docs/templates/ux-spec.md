...
> **Related PRDs**: [Links to the PRD sections that generated this UI requirement — e.g., `design/prd/inventory.md § UI Requirements`]
...
> **Note — Scope boundary**: This template covers discrete screens and flows (list
> views, detail screens, settings, dialogs, multi-step forms). For dashboards
> (multiple widgets summarizing state at a glance), use `dashboard-design.md`
> instead. If a screen is a hybrid (e.g., a settings screen with a small status
> widget), treat it as a screen spec and note the embedded widget in the Anatomy
> section.
...
> user's perspective. Screens that are designed from a developer perspective ("display
> the save data") produce cluttered, confusing interfaces. Screens designed from the
> user's perspective ("let the user feel confident their progress is safe before they
...
a user say they want when they open this screen? What would frustrate them if it did
not work? That frustration describes the need.

Example — bad: "Displays the user's current items and equipment."
Example — good: "Lets the user understand what they're carrying and quickly decide what
to take into the next workflow step, without breaking their mental model of the product world.
The inventory is the user's planning tool between moments of action."]

**The user goal** (what the user wants to accomplish):
...
user's equipment choices and relay them to the core-feature interaction system before the next workflow step
loads." This section prevents UI that looks good but fails to serve the system it is
part of.]

---

## 2. User Context on Arrival

> **Why this section exists**: Screens do not exist in isolation. A user opening the
> inventory mid-core-feature interaction is in a completely different cognitive and emotional state than
> a user opening it after clearing a dungeon. The same information architecture can
...
> what to simplify — are calibrated to the actual user arriving at this screen, not
> an abstract user.

| Question | Answer |
|----------|--------|
| What was the user just doing? | [e.g., Completed a core-feature interaction workflow step / Pressed Esc from exploration / Triggered a story cutscene] |
| What is their emotional state? | [e.g., High tension — just narrowly survived / Calm — exploring between objectives] |
| What cognitive load are they carrying? | [e.g., High — actively tracking edge case positions / Low — no active threats] |
| What information do they already have? | [e.g., They know they just picked up an item but haven't seen its stats yet] |
| What are they most likely trying to do? | [e.g., Check if the new item is better than their current tool — primary use case] |
...
[One sentence describing the feeling the user should have while using this screen.
Example: "Confident and in control — the user should feel like they have complete
...
> behavior, or its relationship to the product's pause state. Navigation position also
...
**Modal behavior**: [Modal (blocks everything behind it, requires explicit dismiss) | Non-modal (product continues behind it) | Overlay (renders over product world, product paused) | Overlay-live (renders over product world, product continues)]
...
> must the user complete it? Undismissable modals are high-friction — justify them.

**Reachability — all entry points**:

| Entry Point | Triggered By | Notes |
|-------------|-------------|-------|
| [e.g., Main Menu → Play] | [User selects "New Product"] | [Primary entry point] |
| [e.g., Pause Menu → Resume] | [User presses Start from any user flow state] | [Secondary entry] |
| [e.g., Product event] | [Tutorial system forces open first time only] | [Systemic entry — must not break if user dismisses] |
...
> Transitions that are undefined become bugs — the user finds themselves stuck, or the
> product state becomes inconsistent. Fill this table completely before implementation
...
|---------|----------------------|-----------------|----------------|-------|
| [e.g., User presses Inventory button] | [User-facing behavior / Exploration state] | [Overlay push — product pauses] | [Current user loadout, inventory contents] | [Works from any non-core-feature interaction state] |
| [e.g., Item pickup prompt accepted] | [User-facing behavior / Item Pickup dialog] | [Replace dialog with full inventory] | [Newly acquired item pre-highlighted] | [The new item should be visually distinguished on open] |
| [e.g., Task system directs user to inventory] | [User-facing behavior / Task Update notification] | [Overlay push] | [Task-relevant item ID for highlight] | [Screen should deep-link to the relevant item] |

**Exit table**:

| Exit Action | Destination | Transition Type | Data Returned / Saved | Notes |
|-------------|------------|-----------------|----------------------|-------|
| [e.g., User closes inventory (Back/B/Esc)] | [Previous state — Exploration] | [Overlay pop — product resumes] | [Updated equipment loadout committed] | [Changes must be committed before transition begins] |
| [e.g., User selects "Equip" on item] | [Same screen, updated state] | [In-place state change] | [Loadout change event fired] | [No navigation, just a state refresh] |
| [e.g., User navigates to Map from inventory shortcut] | [Map Screen] | [Replace] | [No data] | [Inventory state is preserved if user returns] |
...
| [Empty — no items in category] | [User switches to a category with zero items] | [Item Grid replaced by EmptyState component: icon + "Nothing here yet."] | [Action Bar shows no item actions; Drop/Equip/Compare all hidden] | [Do not show disabled buttons — remove them. Disabled buttons with no tooltip are confusing.] |
...
| [Item Selected] | [User navigates to an item slot] | [Detail Panel populates; selected slot has focus ring; Action Bar updates to item's valid actions] | [Equip/Drop/Compare enabled based on item type] | [Equip is disabled if item is already equipped — show a "Equipped" badge instead] |
| [Confirmation Pending — Drop] | [User selects Drop action] | [Confirmation dialog overlays the screen] | [All background interactions suspended until dialog resolves] | [Use a modal confirmation, not an inline toggle. Items cannot be recovered after dropping.] |
| [Error — data load failed] | [Inventory data could not be retrieved] | [Item Grid shows error state: icon + "Couldn't load items." + Retry button] | [Only Retry and Close are available] | [Log the error; do not expose technical details to user] |
| [Item Newly Acquired] | [Screen opened from item pickup deep-link] | [Newly acquired item has a visual "New" badge; Detail Panel pre-populated with that item] | [Same as Item Selected but with badge until user navigates away] | [Badge persists until the user manually navigates off that slot once] |
...
> method (mouse, keyboard, touch) and every interactive state (hover, focus,
...
> only reachable by mouse, it will fail the keyboard column.

### 7.1 Navigation Inputs

| Input | Surface | Action | Visual Response | Notes |
|-------|---------|--------|-----------------|-------|
| [Arrow keys] | [Desktop] | [Move focus within active zone (e.g., row up / down in a list)] | [Focus ring moves to adjacent element] | [Wrap at edges within zone; do not cross zones with arrows alone] |
| [Tab] | [Desktop] | [Move focus to next zone (Filters → List → Detail → Action Bar)] | [Focus ring jumps to first element in next zone] | [Shift+Tab goes backward] |
| [Mouse hover] | [Desktop] | [Show hover state on interactive elements] | [Highlight / underline / color shift] | [Hover does NOT move focus — only click does] |
| [Mouse click] | [Desktop] | [Select and focus the clicked element] | [Pressed state flash, then selected / focused] | [Right-click opens context menu if applicable; otherwise no-op] |
| [Touch tap] | [Mobile] | [Select and activate in one gesture] | [Press ripple] | [Treat tap as click + confirm for low-risk actions; require explicit confirm for destructive actions] |

### 7.2 Action Inputs

| Input | Surface | Context (What must be focused) | Action | Response | Animation | Notes |
|-------|---------|---------------------------------|--------|----------|-----------|-------|
| [Enter / Left click] | [All] | [Row focused] | [Open detail panel] | [Detail panel slides in or updates in place] | [Panel fade / slide in, 120ms] | [If detail already open for this row: no-op] |
| [Enter] | [All] | [Primary action button focused] | [Run primary action (e.g., Save)] | [Button animates press; success toast fires on completion] | [Press, 80ms] | [Fires the corresponding Server Action] |
| [Right-click / long-press] | [All] | [Row focused] | [Open row context menu] | [Context menu appears adjacent to row] | [Popover, 80ms] | [Context menu contains common row actions] |
| [Esc / Back] | [All] | [Any, screen level] | [Close screen / dismiss modal and return to previous state] | [Screen / modal exit transition plays] | [Fade or slide out, 200ms] | [Commits unsaved changes per the screen's draft policy] |
| [Cmd/Ctrl + K] | [Desktop] | [Any] | [Open command palette] | [Palette overlay opens with focus] | [Fade in, 120ms] | [Optional but expected on power-user surfaces] |
...
> **Why this section exists**: The separation between UI and product state is the most
> important architectural boundary in a product's UI system. UI reads data; it does not
...
| [e.g., User currency] | [Economy System] | [On screen open only — inventory does not show live currency] | [EconomySystem] | [Int — gold pieces] | [If currency system not active for this product mode, hide the currency row entirely] |
...
> user actions fire events (see Section 9). Systems update their own data and
> notify the UI.
...
> communicates back to the product. Specifying events at design time prevents UI
> programmers from writing product logic, and prevents product programmers from being
> surprised by what the UI does. Every destructive or state-changing user action
> must appear in this table.

| User Action | Event Fired | Payload | Receiver System | Notes |
|---------------|-------------|---------|-----------------|-------|
| [User equips an item] | [EquipItemRequested] | [{item_id: string, target_slot: string}] | [Equipment System] | [Equipment System validates the action and fires EquipmentChanged if successful; UI listens for EquipmentChanged to update its display] |
| [User drops an item] | [DropItemRequested] | [{item_id: string, quantity: int}] | [Inventory System] | [Fires only after user confirms the drop dialog. Inventory System removes the item and fires InventoryChanged.] |
| [User opens item compare] | [ItemCompareOpened] | [{item_a_id: string, item_b_id: string}] | [Analytics System] | [No product-state change — analytics event only. Compare view is purely local UI state.] |
| [User closes screen] | [InventoryScreenClosed] | [{session_duration_ms: int}] | [Analytics System] | [Fires on every close regardless of reason. Used for engagement metrics.] |
| [User navigates between categories] | [InventoryCategoryChanged] | [{category: string}] | [Analytics System] | [Analytics only. No product state change.] |
...
> user has moved forward. A screen that fades implies a context break. Inconsistent
...
| [Screen exit — Back] | [User presses Back] | [Slide out to right] | [200] | [Ease in cubic] | [No] | [Yes — instant disappear] |
| [Screen exit — Forward] | [User navigates to child screen] | [Slide out to left] | [200] | [Ease in cubic] | [No] | [Yes — instant] |
| [Detail panel update] | [User selects a new item] | [Cross-fade content] | [120] | [Linear] | [Yes — if user navigates quickly, previous animation cancels] | [Yes — instant swap] |
| [Loading → Populated] | [Data arrives after load] | [Skeleton shimmer fades out, content fades in] | [180] | [Ease out] | [No] | [Yes — instant reveal] |
| [Action Bar button press] | [User activates a button] | [Scale down 95% on press, return on release] | [60 down / 60 up] | [Ease out / ease in] | [Yes — if released early, returns to normal] | [No — this is tactile feedback, not decorative motion] |
| [Confirmation dialog open] | [User initiates destructive action] | [Background dims 60% opacity; dialog scales up from 95%] | [150] | [Ease out] | [No] | [Yes — instant appear, no scale] |
...
- [ ] Esc key closes or cancels (and does not quit the product from within a screen)

**Touch**
- [ ] All interactive elements have touch targets ≥ 44×44 CSS px
- [ ] No hover-only behavior — every hover affordance is also reachable via focus or tap
- [ ] Long-press behavior defined for any non-standard interaction
- [ ] Swipe gestures (if used) do not conflict with system-level swipes
- [ ] Orientation change preserves screen state
...
| [User focuses an item slot] | ["[Item name]. [Category]. [Rarity]. [Key stats summary]. [Equipped / Not equipped]."] | [On focus arrival] |
| [User equips an item] | ["[Item name] equipped to [slot name]."] | [After EquipmentChanged event confirmed] |
| [User drops an item] | ["[Item name] dropped."] | [After InventoryChanged event confirmed] |
...
[Estimate the number of information streams the user is simultaneously tracking while
...
Mitigation: detail panel auto-updates on navigation so the user never needs to
...
- [ ] All interactive elements reachable by touch on mobile / tablet viewports
...
- [ ] Screen does not write directly to any product system (verify: no direct state mutation calls)
...
|----------|-------|----------|-----------|
| [e.g., Should item comparison be automatic (always showing equipped stats) or user-triggered (press Compare)?] | [ui-designer] | [Sprint 4, Day 3] | [Pending] |
| [e.g., Do we support controller cursor (free aim) in the item grid, or d-pad-only grid navigation?] | [lead-engineer + ui-designer] | [Sprint 4, Day 3] | [Pending — depends on ADR-0015 input model decision] |
| [e.g., What is the product's item drop policy — permanent loss or drop-to-world?] | [systems-analyst] | [Requires PRD update] | [Blocked on inventory PRD Edge Cases section] |
| [e.g., Maximum inventory size — does the grid have a hard cap or is it infinite-scroll?] | [business-analyst] | [Sprint 3, Day 5] | [Pending] |
