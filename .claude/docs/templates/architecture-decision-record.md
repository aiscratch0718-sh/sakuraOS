...
framework version and design. Update this date when you re-read and confirm it
...
| **References Consulted** | [e.g. `docs/framework-reference/nextjs/modules/physics.md`, `breaking-changes.md`] |
| **Post-Cutoff APIs Used** | [Specific APIs from post-cutoff framework versions this decision depends on, or "None"] |
| **Verification Required** | [Concrete behaviours to test against the target framework version before shipping, or "None"] |

> **Note**: If Knowledge Risk is MEDIUM or HIGH, this ADR must be re-validated if the
> project upgrades framework versions. Flag it as "Superseded" and write a new ADR.
...
- [Technical constraints -- framework limitations, platform requirements]
...
<!-- This section is MANDATORY. Every ADR must trace back to at least one PRD
 requirement, or explicitly state it is a foundational decision with no PRD
 dependency. Traceability is audited by /architecture-review. -->

| PRD Document | System | Requirement | How This ADR Satisfies It |
|-------------|--------|-------------|--------------------------|
| [e.g. `design/prd/core-feature interaction.md`] | [e.g. Core feature interaction] | [e.g. "Hitbox detection must resolve within 1 frame"] | [e.g. "Jolt physics collision queries run synchronously in _physics_process"] |

> If this is a foundational decision with no direct PRD dependency, write:
> "Foundational — no PRD requirement. Enables: [list what PRD systems this
